...
class CMakeBuildExt(build_ext):
    ...
    def build_cmake(self, ext: CMakeExtension):
        ...
        if ext.backend == "cuda":
            ...
            cmake_args.append("-DCMAKE_HIP_FLAGS=-Wno-error=attributes")
            cmake_args.append(f"-DPython_INCLUDE_DIR={sysconfig.get_config_var('INCLUDEPY')}")
            cmake_args.append(
                f"-DPython_LIBRARY={sysconfig.get_config_var('LIBDIR')}/libpython3.14.so"
            )
        else:
...
def get_rocm_path() -> tuple[str | None, pathlib.Path | None]:
    ...
    if rocm_home:
        ...
        for candidate in (
            rocm_path / "llvm" / "bin" / "clang++",  # Fedora ROCm path
            rocm_path / "bin" / "amdclang++",  # AMD upstream path
            rocm_path / "bin" / "clang++",  # fallback
            rocm_path / "bin" / "hipcc",  # last resort
        ):
    ...
    if compiler is None:
        for name in ("clang++", "amdclang++", "hipcc"):
            compiler_path = shutil.which(name)
            if compiler_path:
                # compiler = pathlib.Path(compiler_path)
                compiler = pathlib.Path("/usr/lib64/rocm/llvm/bin/clang++")  # HARDCODED
