...
extern "C"
{
        void launch_quantize_fp8_kernel(const void* input, void* output, const void* scale,
                                        int64_t numel, int input_dtype_code, int output_dtype_code,
                                        hipStream_t stream);

        void launch_dequantize_fp8_kernel(const void* input, void* output, const void* scale,
                                          int64_t numel, int input_dtype_code,
        ...
        void launch_cublas_gemm_blockwise_fp4_kernel(
            ...
            int64_t K, const float* alpha_device_ptr, int out_dtype_code, void* workspace_ptr,
            bool accumulate, hipStream_t stream);
        ...
        void launch_svdquant_quantize_w4a4_kernel(const void* x, const void* smooth,
                                                  const void* lora_down, void* q_x, void* ascales,
                                                  void* lora_act, int M, int M_pad, int K, int R,
                                                  int input_dtype_code, int act_unsigned,
                                                  hipStream_t stream);
        ...
        void launch_svdquant_scaled_mm_w4a4_kernel(
            ...
            const void* lora_act_in, const void* lora_up, const void* bias, void* out, int M, int N,
            int K, int R, int act_unsigned, int out_dtype_code, int tile_packed, int fast_accum,
            int shared_scale, int fuse_lora, hipStream_t stream);
                               ...
                               int output_dtype_code, int64_t numel, uintptr_t stream_ptr)
{
        ...
        if (input_dtype_code != 5 && input_dtype_code != 6)
        {
                throw std::runtime_error(
                    "Unsupported input dtype code for dequantize_per_tensor_fp8 (must be 5 or 6)");
        }
        ...
        {
                throw std::runtime_error(
                    "Unsupported output dtype for dequantize_per_tensor_fp8 (must be float32, "
                    "float16, or bfloat16)");
        }
                          ...
                          uintptr_t stream_ptr)
{
        ...
        launch_stochastic_round_fp8_kernel(rng_and_output.data(), input.data(), numel,
                                           rng_dtype_code, input_dtype_code, output_dtype_code,
                                           stream);
}
                               ...
                               nb::ndarray<float, nb::device::rocm> alpha, uintptr_t stream_ptr)
{
        ...
        launch_cublas_gemm_blockwise_fp4_kernel(
            b.data(), block_scale_b.data(), a.data(), block_scale_a.data(), out.data(), bias_ptr, M,
            N, K, static_cast<const float*>(alpha.data()), out_dtype_code, workspace.data(),
            accumulate, stream);
}
                    ...
                    bool hi_first, uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "Unsupported input dtype for FP4 quantization (must be float32, float16, or "
                    "bfloat16)");
        }

        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_quantize_nvfp4_kernel(input.data(), global_scale.data(), output.data(),
                                     block_scales.data(), num_rows, num_cols, orig_rows, orig_cols,
                                     epsilon, input_dtype_code, hi_first, stream);
}
                      ...
                      bool hi_first, uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "Unsupported output dtype for FP4 dequantization (must be float32, float16, or "
                    "bfloat16)");
        }

        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_dequantize_nvfp4_kernel(input.data(), global_scale.data(), block_scales.data(),
                                       output.data(), num_rows, num_cols, output_dtype_code,
                                       hi_first, stream);
}
...
//         {
//                 throw std::runtime_error(
//                     "Unsupported input dtype for MXFP8 quantization (must be float32, float16, or
//                     " "bfloat16)");
//         }
//
//         hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
//         launch_quantize_mxfp8_kernel(input.data(), output.data(), block_scales.data(), num_rows,
                ...
                uintptr_t stream_ptr, bool split_half = false)
{
        ...
        if (xq_out.ndim() != 4 || xq_out.shape(0) != batch || xq_out.shape(1) != dim1 ||
        ...
        if (has_xk)
        {
                ...
                if (xk_out.ndim() != 4 || xk_out.shape(0) != batch || xk_out.shape(1) != dim1 ||
    ...
    bool act_unsigned, uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_svdquant_quantize_w4a4_kernel(x.data(), smooth.data(), lora_down.data(), q_x.data(),
               ...
               int group_size, uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_awq_w4a16_kernel(x.data(), qweight.data(), wscales.data(), wzeros.data(), out.data(),
...
extern "C"
{
        void launch_cublas_gemm_int8_kernel(const void* A_ptr, const void* B_ptr, void* C_ptr,
        ...
        void launch_dequantize_int4_convrot64_kernel(const void* input, const void* scales,
                                                     void* output, int64_t M, int64_t K,
                      ...
                      nb::ndarray<nb::device::rocm> workspace, uintptr_t stream_ptr)
{
        ...
        launch_cublas_gemm_int8_kernel(a.data(), b.data(), c.data(), M, N, K, workspace.data(),
                           ...
                           bool stochastic, uint64_t seed, uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_quantize_int8_rowwise_kernel(input.data(), output.data(), scales.data(), M, K,
                           ...
                           bool stochastic, uint64_t seed, uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_quantize_int4_rowwise_kernel(input.data(), output.data(), scales.data(), M, K,
                                            input_dtype_code, stochastic, seed, stream);
}
                                     ...
                                     uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "INT4 ConvRot quantization requires group_size 16, 64, or 256");
        }
        if (K % group_size != 0)
        {
                throw std::runtime_error(
                    "INT4 ConvRot quantization requires K divisible by group_size");
        }
        if (output.shape(0) != M || output.shape(1) != K / 2)
        {
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_quantize_int4_rowwise_convrot64_kernel(input.data(), output.data(), scales.data(), M,
                                                      K, group_size, input_dtype_code, stochastic,
                                                      seed, stream);
}
    ...
    uint64_t seed, uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "INT4 ConvRot fallback activation scales must have shape [M, 1]");
        }
        if (group_size != 256 || K % group_size != 0)
        {
                throw std::runtime_error(
                    "INT4 ConvRot fallback activation quantization requires group_size 256 and "
                    "divisible K");
        }
        const int input_dtype_code = map_dtype_to_code(input.dtype());
        if (input_dtype_code < 0 || input_dtype_code > 2)
        {
                throw std::runtime_error(
                    "Unsupported input dtype for INT4 ConvRot fallback activation quantization");
        }
                               ...
                               uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "INT4 ConvRot dequantization requires group_size 16, 64, or 256");
        }
        if (K % group_size != 0)
        {
                throw std::runtime_error(
                    "INT4 ConvRot dequantization requires K divisible by group_size");
        }
        if (output.shape(0) != M || output.shape(1) != K)
        {
        ...
        {
                throw std::runtime_error(
                    "INT4 ConvRot dequantization scale must be scalar or per-row");
        }
        const int output_dtype_code = map_dtype_to_code(output.dtype());
        if (output_dtype_code < 0 || output_dtype_code > 2)
        {
                throw std::runtime_error(
                    "Unsupported output dtype for INT4 ConvRot dequantization");
        }

        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_dequantize_int4_convrot64_kernel(input.data(), scales.data(), output.data(), M, K,
                                                static_cast<int64_t>(scales.size()), group_size,
                 ...
                 uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "INT4 linear weight_scales must have one value per output channel");
        }
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_int4_linear_kernel(act.data(), weight.data(), x_scales.data(), weight_scales.data(),
                                  has_bias ? bias.data() : nullptr, output.data(), M, N, K,
                                       ...
                                       int output_dtype_code, uintptr_t stream_ptr)
{
        ...
        if (has_bias)
        {
                ...
                {
                        throw std::runtime_error(
                            "Unsupported bias dtype for packed INT4 weight GEMV");
                }
        }

        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_int4_weight_int8_act_gemv_dequant_kernel(
            input.data(), weight.data(), x_scales.data(), weight_scales.data(),
            has_bias ? bias.data() : nullptr, output.data(), M, N, K,
    ...
    bool allow_sm80_cutlass, int output_dtype_code, uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "chunked INT4 weight GEMM activation scale shape mismatch");
        }
        if (output.shape(0) != M || output.shape(1) != N)
        {
        ...
        {
                throw std::runtime_error(
                    "chunked INT4 weight GEMM weight workspace shape mismatch");
        }
        if (acc_workspace.shape(0) != M || acc_workspace.shape(1) < chunk_cols)
        {
                throw std::runtime_error(
                    "chunked INT4 weight GEMM accumulator workspace shape mismatch");
        }
        ...
        if (has_bias)
        {
                ...
                {
                        throw std::runtime_error(
                            "Unsupported bias dtype for chunked INT4 weight GEMM");
                }
        }
                                   ...
                                   uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_quantize_int8_rowwise_convrot_kernel(input.data(), output.data(), scales.data(), M,
                                ...
                                int64_t group_size, uintptr_t stream_ptr)
{
        ...
        const int input_dtype_code = map_dtype_to_code(input.dtype());
        const int output_dtype_code = map_dtype_to_code(output.dtype());
        if (input_dtype_code < 0 || input_dtype_code > 2 || output_dtype_code < 0 ||
            output_dtype_code > 2)
        {
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_rotate_int8_convrot_weight_kernel(input.data(), output.data(), M, K,
                                                 static_cast<int>(group_size), input_dtype_code,
                                                 output_dtype_code, stream);
}
                                  ...
                                  uintptr_t stream_ptr)
{
        ...
        if (input_dtype_code < 0 || input_dtype_code > 2 || rotated_dtype_code < 0 ||
            rotated_dtype_code > 2)
        {
        ...
        launch_quantize_int8_convrot_staged_kernel(
            input.data(), rotated.data(), partial_absmax.data(), output.data(), scales.data(), M, K,
            static_cast<int>(group_size), input_dtype_code, rotated_dtype_code, stochastic, seed,
            stream);
}
                                     ...
                                     uint64_t seed, uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "Unsupported input dtype for INT8 rowwise convrot64 quantization");
        }
                            ...
                            int output_dtype_code, uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_dequantize_int8_linear_kernel(input.data(), x_scales.data(), weight_scales.data(),
                                             has_bias ? bias.data() : nullptr, output.data(), M, N,
                                             static_cast<int64_t>(weight_scales.size()), has_bias,
                                             output_dtype_code, bias_dtype_code, stream);
}
                       ...
                       uintptr_t stream_ptr)
{
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_int8_gemv_dequant_kernel(input.data(), weight.data(), x_scales.data(),
                                        weight_scales.data(), has_bias ? bias.data() : nullptr,
                                        output.data(), N, K,
                                        static_cast<int64_t>(weight_scales.size()), has_bias,
                                        output_dtype_code, bias_dtype_code, stream);
}
                    ...
                    bool convrot, int group_size, uintptr_t stream_ptr)
{
        ...
        {
                launch_quantize_int8_rowwise_kernel(input.data(), q_scratch.data(), x_scales.data(),
                                                    M, K, input_dtype_code, false, 0, stream);
        }
        launch_int8_gemv_dequant_kernel(q_scratch.data(), weight.data(), x_scales.data(),
                                        weight_scales.data(), has_bias ? bias.data() : nullptr,
                                        output.data(), N, K,
                                        static_cast<int64_t>(weight_scales.size()), has_bias,
                                        output_dtype_code, bias_dtype_code, stream);
                            ...
                            uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "INT8 simple dequantization elementwise scale shape mismatch");
        }
        if (scale_mode == 2 && (inner_dim <= 0 || input.size() % inner_dim != 0 ||
                                scale.size() != input.size() / inner_dim))
        {
        ...
        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_dequantize_int8_simple_kernel(input.data(), scale.data(), output.data(),
                                             static_cast<int64_t>(input.size()), inner_dim,
                                    ...
                                    int64_t group_size, uintptr_t stream_ptr)
{
        ...
        {
                throw std::runtime_error(
                    "Unsupported output dtype for INT8 convrot dequantization");
        }

        hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
        launch_dequantize_int8_convrot_kernel(
            input.data(), scale.data(), output.data(), M, K, static_cast<int64_t>(scale.size()),
            static_cast<int>(group_size), output_dtype_code, stream);
}

NB_MODULE(_C, m)
{
        m.doc() =
            "comfy_kitchen CUDA kernels - nanobind + DLPack interface (NO PyTorch C++ "
            "dependencies)";

        m.def("quantize_per_tensor_fp8", &quantize_per_tensor_fp8,
              "Quantize to FP8 using nanobind ndarrays", nb::arg("input"), nb::arg("scale"),
              nb::arg("output"), nb::arg("input_dtype_code"), nb::arg("output_dtype_code"),
              nb::arg("numel"), nb::arg("stream_ptr"));

        m.def("dequantize_per_tensor_fp8", &dequantize_per_tensor_fp8,
              "Dequantize from FP8 using nanobind ndarrays", nb::arg("input"), nb::arg("scale"),
              nb::arg("output"), nb::arg("input_dtype_code"), nb::arg("output_dtype_code"),
              nb::arg("numel"), nb::arg("stream_ptr"));

        m.def("stochastic_round_fp8", &stochastic_round_fp8,
              "Stochastically round to FP8, overwriting RNG storage with FP8 output",
              nb::arg("rng_and_output"), nb::arg("input"), nb::arg("output_dtype_code"),
              nb::arg("numel"), nb::arg("stream_ptr"));

        m.def("cublas_gemm_blockwise_fp4", &cublas_gemm_blockwise_fp4,
              "cuBLAS FP4 GEMM with block-wise scaling", nb::arg("b"), nb::arg("block_scale_b"),
              nb::arg("a"), nb::arg("block_scale_a"), nb::arg("out"), nb::arg("out_dtype_code"),
              nb::arg("bias"), nb::arg("workspace"), nb::arg("accumulate"), nb::arg("alpha"),
              nb::arg("stream_ptr"));

        m.def("cublas_gemm_int8", &cublas_gemm_int8,
              "INT8 GEMM using cuBLASLt IMMA tensor cores (SM >= 7.5)", nb::arg("a"), nb::arg("b"),
              nb::arg("c"), nb::arg("workspace"), nb::arg("stream_ptr"));

        m.def("quantize_int8_rowwise", &quantize_int8_rowwise,
              "Rowwise INT8 quantization for CUDA activations", nb::arg("input"), nb::arg("output"),
              nb::arg("scales"), nb::arg("stochastic"), nb::arg("seed"), nb::arg("stream_ptr"));

        m.def("quantize_int4_rowwise", &quantize_int4_rowwise,
              "Rowwise signed INT4 quantization for CUDA activations/weights", nb::arg("input"),
              nb::arg("output"), nb::arg("scales"), nb::arg("stochastic"), nb::arg("seed"),
              nb::arg("stream_ptr"));
        ...
        m.def("int4_linear", &int4_linear,
              ...
              nb::arg("act"), nb::arg("weight"), nb::arg("x_scales"), nb::arg("weight_scales"),
        ...
        m.def("rotate_int8_convrot_weight", &rotate_int8_convrot_weight,
              "ConvRot Hadamard weight rotation", nb::arg("input"), nb::arg("output"),
        ...
        m.def("quantize_int8_convrot_staged", &quantize_int8_convrot_staged,
              ...
              nb::arg("input"), nb::arg("rotated"), nb::arg("partial_absmax"), nb::arg("output"),
              nb::arg("scales"), nb::arg("group_size"), nb::arg("stochastic"), nb::arg("seed"),
              nb::arg("stream_ptr"));
        ...
        m.def("dequantize_int8_linear", &dequantize_int8_linear,
              ...
              nb::arg("x_scales"), nb::arg("weight_scales"), nb::arg("bias"), nb::arg("output"),
        ...
        m.def("int8_linear_m1", &int8_linear_m1,
              "M=1 INT8 linear: activation quantization followed by GEMV/dequant", nb::arg("input"),
              nb::arg("q_scratch"), nb::arg("x_scales"), nb::arg("weight"),
              nb::arg("weight_scales"), nb::arg("bias"), nb::arg("output"),
              nb::arg("output_dtype_code"), nb::arg("convrot"), nb::arg("group_size"),
              nb::arg("stream_ptr"));

        m.def("dequantize_int8_simple", &dequantize_int8_simple, "INT8 dequantization to float32",
              nb::arg("input"), nb::arg("scale"), nb::arg("output"), nb::arg("inner_dim"),
              nb::arg("scale_mode"), nb::arg("stream_ptr"));

        m.def("dequantize_int8_convrot_weight", &dequantize_int8_convrot_weight,
              "INT8 ConvRot weight dequantization to float32", nb::arg("input"), nb::arg("scale"),
              nb::arg("output"), nb::arg("group_size"), nb::arg("stream_ptr"));
        ...
        m.def("quantize_nvfp4", &quantize_nvfp4,
              ...
              nb::arg("input"), nb::arg("global_scale"), nb::arg("output"), nb::arg("block_scales"),
              nb::arg("epsilon"), nb::arg("pad_16x") = false, nb::arg("hi_first") = true,
              nb::arg("stream_ptr"));

        m.def("dequantize_nvfp4", &dequantize_nvfp4,
              "Dequantize from FP4 E2M1 with E4M3 block scales using cuBLAS tiled layout",
              nb::arg("input"), nb::arg("global_scale"), nb::arg("block_scales"), nb::arg("output"),
