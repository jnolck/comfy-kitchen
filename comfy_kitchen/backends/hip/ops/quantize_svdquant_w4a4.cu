...
template<>
__device__ __forceinline__ float to_fp32<__hip_bfloat16>(__hip_bfloat16 v) { return __bfloat162float(v); }
...
template<>
__device__ __forceinline__ __hip_bfloat16 fp32_to_in<__hip_bfloat16>(float v) { return __float2bfloat16(v); }
    ...
    hipStream_t stream)
{
    ...
    if (input_dtype_code == 2) {
        if (act_unsigned) LAUNCH_QUANTIZE(__hip_bfloat16, true);
        else              LAUNCH_QUANTIZE(__hip_bfloat16, false);
