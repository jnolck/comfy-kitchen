...
template <typename InputType, typename FreqsType, typename ScaleType, bool HasRms, bool SplitHalf,
          bool HasK, bool InPlace, bool ContigHead>
__global__ __launch_bounds__(kThreads) void rope_kernel(
    const InputType* q, const InputType* k, const FreqsType* __restrict__ freqs,
    const ScaleType* __restrict__ q_scale, const ScaleType* __restrict__ k_scale, InputType* q_out,
    InputType* k_out, int64_t batch, int64_t dim1, int64_t dim2, int head_dim, int rot_dim,
    int64_t freqs_batch, int64_t freqs_dim1, int64_t freqs_dim2, int64_t q_s0, int64_t q_s1,
    int64_t q_s2, int64_t q_s3, int64_t k_s0, int64_t k_s1, int64_t k_s2, int64_t k_s3,
    int64_t qo_s0, int64_t qo_s1, int64_t qo_s2, int64_t qo_s3, int64_t ko_s0, int64_t ko_s1,
    ...
    int64_t f_s4, int64_t f_s5, int64_t qs_stride, int64_t ks_stride, float epsilon)
{
        ...
        const int64_t qo_base = InPlace ? q_base : i0 * qo_s0 + i1 * qo_s1 + i2 * qo_s2;
        int64_t k_base = 0;
        int64_t ko_base = 0;
        if constexpr (HasK)
        {
                k_base = i0 * k_s0 + i1 * k_s1 + i2 * k_s2;
                ko_base = InPlace ? k_base : i0 * ko_s0 + i1 * ko_s1 + i2 * ko_s2;
        }
        ...
        if constexpr (HasRms)
        {
                ...
                {
                        const float k_sum =
                            rope::rms_sum<InputType, ContigHead>(k + k_base, head_dim, k_s3, lane);
...
#pragma unroll
                for (int p = 0; p < kPairsPerLane; ++p)
                {
                        ...
                        ComputeType q0 = static_cast<ComputeType>(static_cast<float>(q0_raw[p]));
                        ComputeType q1 = static_cast<ComputeType>(static_cast<float>(q1_raw[p]));
                        ...
                        const ComputeType f00 =
                            static_cast<ComputeType>(static_cast<float>(f00_raw));
                        const ComputeType f01 =
                            static_cast<ComputeType>(static_cast<float>(f01_raw));
                        const ComputeType f10 =
                            static_cast<ComputeType>(static_cast<float>(f10_raw));
                        const ComputeType f11 =
                            static_cast<ComputeType>(static_cast<float>(f11_raw));
                        ComputeType qo0, qo1;
                        rope::rotate(q0, q1, f00, f01, f10, f11, qo0, qo1);
                        qo0_raw[p] = static_cast<InputType>(static_cast<float>(qo0));
                        qo1_raw[p] = static_cast<InputType>(static_cast<float>(qo1));

                        if constexpr (HasK)
                        {
                                ComputeType k0 =
                                    static_cast<ComputeType>(static_cast<float>(k0_raw[p]));
                                ComputeType k1 =
                                    static_cast<ComputeType>(static_cast<float>(k1_raw[p]));
                                if constexpr (HasRms)
                                {
                                        ...
                                        k1 = static_cast<float>(static_cast<InputType>(
                                            ...
                                            static_cast<float>(
                                                k_scale[static_cast<int64_t>(second) *
                                                        ks_stride])));
                                }
                                ComputeType ko0, ko1;
                                rope::rotate(k0, k1, f00, f01, f10, f11, ko0, ko1);
                                ko0_raw[p] = static_cast<InputType>(static_cast<float>(ko0));
                                ko1_raw[p] = static_cast<InputType>(static_cast<float>(ko1));
                        }
                }
                // /////////////////////////////////////////////////////////////////////////////////
                ...
                // {
                //         printf(
                //             "rms_rope debug: batch=%lld, dim1=%lld, dim2=%lld, head_dim=%d, "
                //             "rot_dim=%d\n",
                //             (long long)batch, (long long)dim1, (long long)dim2, head_dim,
                //             rot_dim);
                //         printf(
                //             "rms_rope debug: pairs=%d, SplitHalf=%d, HasRms=%d, HasK=%d, "
                //             "InPlace=%d, ContigHead=%d\n",
                //             pairs, (int)SplitHalf, (int)HasRms, (int)HasK, (int)InPlace,
                //             (int)ContigHead);
                //         printf("rms_rope debug: q_s3=%lld, qo_s3=%lld, k_s3=%lld, ko_s3=%lld\n",
                //                (long long)q_s3, (long long)qo_s3, (long long)k_s3,
                //                (long long)ko_s3);
                //         printf("rms_rope debug: q_rrms=%f, k_rrms=%f, epsilon=%f\n", q_rrms,
                //         k_rrms,
                //                epsilon);
                //         // Sample first few input/output values to see if reads are right
                //         printf("rms_rope debug: q[0]=%f, q_out[0]=%f\n",
                //                static_cast<float>(q[q_base]),
                //                static_cast<float>(q_out[qo_base]));
                // }
                ...
                else
                {
                        rope::store_head_pair<InputType, SplitHalf, ContigHead>(
                            q_out + qo_base, pair_base, pairs, InPlace ? q_s3 : qo_s3, qo0_raw[0],
                            qo1_raw[0]);
                        if constexpr (HasK)
                        {
                                rope::store_head_pair<InputType, SplitHalf, ContigHead>(
                ...
                if constexpr (HasK)
                {
                        ...
                        if constexpr (HasRms)
                        {
                                kv = static_cast<InputType>(
                                    ...
                                    static_cast<float>(
                                        k_scale[static_cast<int64_t>(d) * ks_stride]));
                        }
...
{
        return reinterpret_cast<uintptr_t>(ptr) % alignof(rope::Pair<T>) == 0 && s0 % 2 == 0 &&
               s1 % 2 == 0 && s2 % 2 == 0;
}

template <typename InputType, typename FreqsType, typename ScaleType, bool HasRms, bool SplitHalf,
          bool HasK, bool InPlace, bool ContigHead>
void launch_config(const InputType* q, const InputType* k, const FreqsType* freqs,
                   ...
                   int rot_dim, int64_t freqs_batch, int64_t freqs_dim1, int64_t freqs_dim2,
                   ...
                   hipStream_t stream)
{
        ...
        const int blocks = static_cast<int>((rows + kWarpsPerBlock - 1) / kWarpsPerBlock);
        rope_kernel<InputType, FreqsType, ScaleType, HasRms, SplitHalf, HasK, InPlace, ContigHead>
            <<<blocks, kThreads, 0, stream>>>(
                q, k, freqs, q_scale, k_scale, q_out, k_out, batch, dim1, dim2, head_dim, rot_dim,
                freqs_batch, freqs_dim1, freqs_dim2, q_s0, q_s1, q_s2, q_s3, k_s0, k_s1, k_s2, k_s3,
                qo_s0, qo_s1, qo_s2, qo_s3, ko_s0, ko_s1, ko_s2, ko_s3, f_s0, f_s1, f_s2, f_s3,
                f_s4, f_s5, qs_stride, ks_stride, epsilon);
}

template <typename InputType, typename FreqsType, typename ScaleType, bool HasRms>
void rope_launcher(const InputType* q, const InputType* k, const FreqsType* freqs,
                   const ScaleType* q_scale, const ScaleType* k_scale, InputType* q_out,
                   InputType* k_out, int64_t batch, int64_t dim1, int64_t dim2, int head_dim,
                   int rot_dim, int64_t freqs_batch, int64_t freqs_dim1, int64_t freqs_dim2,
                   int64_t q_s0, int64_t q_s1, int64_t q_s2, int64_t q_s3, int64_t k_s0,
                   ...
                   bool has_k, bool split_half, hipStream_t stream)
{
        ...
        bool contig = q_s3 == 1 && qo_s3 == 1 && pair_aligned(q, q_s0, q_s1, q_s2) &&
                      pair_aligned(q_out, qo_s0, qo_s1, qo_s2);
        if (has_k)
        {
                contig = contig && k_s3 == 1 && ko_s3 == 1 && pair_aligned(k, k_s0, k_s1, k_s2) &&
...
#define LAUNCH(HAS_K, SPLIT, INPLACE, CONTIG)                                                     \
        launch_config<InputType, FreqsType, ScaleType, HasRms, SPLIT, HAS_K, INPLACE, CONTIG>(    \
            q, k, freqs, q_scale, k_scale, q_out, k_out, batch, dim1, dim2, head_dim, rot_dim,    \
            freqs_batch, freqs_dim1, freqs_dim2, q_s0, q_s1, q_s2, q_s3, k_s0, k_s1, k_s2, k_s3,  \
            qo_s0, qo_s1, qo_s2, qo_s3, ko_s0, ko_s1, ko_s2, ko_s3, f_s0, f_s1, f_s2, f_s3, f_s4, \
            f_s5, qs_stride, ks_stride, epsilon, stream)
#define DISPATCH_LAYOUT(HAS_K, SPLIT)                       \
        if (inplace)                                        \
        {                                                   \
                if (contig)                                 \
                        LAUNCH(HAS_K, SPLIT, true, true);   \
                else                                        \
                        LAUNCH(HAS_K, SPLIT, true, false);  \
        }                                                   \
        else                                                \
        {                                                   \
                if (contig)                                 \
                        LAUNCH(HAS_K, SPLIT, false, true);  \
                else                                        \
                        LAUNCH(HAS_K, SPLIT, false, false); \
        }
        if (has_k)
        {
                if (split_half)
...
extern "C" void launch_apply_rope_kernel(
    const void* q, const void* k, const void* freqs, void* q_out, void* k_out, int64_t batch,
    int64_t dim1, int64_t dim2, int64_t head_dim, int64_t freqs_batch, int64_t freqs_dim1,
    int64_t freqs_dim2, int64_t q_s0, int64_t q_s1, int64_t q_s2, int64_t q_s3, int64_t k_s0,
    int64_t k_s1, int64_t k_s2, int64_t k_s3, int64_t qo_s0, int64_t qo_s1, int64_t qo_s2,
    int64_t qo_s3, int64_t ko_s0, int64_t ko_s1, int64_t ko_s2, int64_t ko_s3, int64_t f_s0,
    int64_t f_s1, int64_t f_s2, int64_t f_s3, int64_t f_s4, int64_t f_s5, int input_dtype_code,
    int freqs_dtype_code, bool has_k, bool split_half, hipStream_t stream)
{
        DISPATCH_HALF_INPUT_FP_FREQS_DTYPES(
            ...
            [&]
            {
                    comfy::rope_launcher<InputType, FreqsType, float, false>(
                        ...
                        static_cast<InputType*>(q_out), static_cast<InputType*>(k_out), batch, dim1,
                        dim2, static_cast<int>(head_dim), static_cast<int>(head_dim), freqs_batch,
                        freqs_dim1, freqs_dim2, q_s0, q_s1, q_s2, q_s3, k_s0, k_s1, k_s2, k_s3,
                        qo_s0, qo_s1, qo_s2, qo_s3, ko_s0, ko_s1, ko_s2, ko_s3, f_s0, f_s1, f_s2,
...
extern "C" void launch_rms_rope_kernel(
    ...
    void* q_out, void* k_out, int64_t batch, int64_t dim1, int64_t dim2, int64_t head_dim,
    int64_t rot_dim, int64_t freqs_batch, int64_t freqs_dim1, int64_t freqs_dim2, int64_t q_s0,
    int64_t q_s1, int64_t q_s2, int64_t q_s3, int64_t k_s0, int64_t k_s1, int64_t k_s2,
    int64_t k_s3, int64_t qo_s0, int64_t qo_s1, int64_t qo_s2, int64_t qo_s3, int64_t ko_s0,
    int64_t ko_s1, int64_t ko_s2, int64_t ko_s3, int64_t f_s0, int64_t f_s1, int64_t f_s2,
    int64_t f_s3, int64_t f_s4, int64_t f_s5, int64_t qs_stride, int64_t ks_stride, float epsilon,
    int input_dtype_code, int freqs_dtype_code, int scale_dtype_code, bool has_k, bool split_half,
    hipStream_t stream)
{
        DISPATCH_HALF_DTYPE(
            ...
            {
                    DISPATCH_FP_DTYPE(
                        freqs_dtype_code, FreqsType,
                        [&]
                        {
                                DISPATCH_FP_DTYPE(
                                    scale_dtype_code, ScaleType,
                                    [&]
                                    {
                                            comfy::rope_launcher<InputType, FreqsType, ScaleType,
                                                                 true>(
                                                ...
                                                static_cast<InputType*>(q_out),
                                                static_cast<InputType*>(k_out), batch, dim1, dim2,
                                                static_cast<int>(head_dim),
                                                static_cast<int>(rot_dim), freqs_batch, freqs_dim1,
                                                freqs_dim2, q_s0, q_s1, q_s2, q_s3, k_s0, k_s1,
                                                k_s2, k_s3, qo_s0, qo_s1, qo_s2, qo_s3, ko_s0,
                                                ko_s1, ko_s2, ko_s3, f_s0, f_s1, f_s2, f_s3, f_s4,
                                                f_s5, qs_stride, ks_stride, epsilon, has_k,
                                                split_half, stream);
                                    });
                        });
            });
