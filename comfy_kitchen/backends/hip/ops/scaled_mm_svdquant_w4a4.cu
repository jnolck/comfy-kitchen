...
template <typename OutType> __device__ __forceinline__ OutType fp32_to_out(float v);

template <> __device__ __forceinline__ __hip_bfloat16 fp32_to_out<__hip_bfloat16>(float v) {
    return __float2bfloat16(v);
}

template <> __device__ __forceinline__ __half fp32_to_out<__half>(float v) {
    return __float2half(v);
}

template <typename OutType> __device__ __forceinline__ float load_scale(const OutType * p) {
    if constexpr (std::is_same_v<OutType, __hip_bfloat16>) {
...
template <typename OutType> __device__ __forceinline__ float2 load_scale2(const OutType * p);

template <> __device__ __forceinline__ float2 load_scale2<__hip_bfloat16>(const __hip_bfloat16 * p) {
    return __bfloat1622float2(*reinterpret_cast<const __hip_bfloat162 *>(p));
}

template <> __device__ __forceinline__ float2 load_scale2<__half>(const __half * p) {
    return __half22float2(*reinterpret_cast<const __half2 *>(p));
}

template <typename OutType> struct Vec2Traits;

template <> struct Vec2Traits<__half> {
    using Pair = __half2;

    __device__ __forceinline__ static Pair zero() { return __floats2half2_rn(0.f, 0.f); }

    __device__ __forceinline__ static Pair from_floats(float x, float y) { return __floats2half2_rn(x, y); }

    __device__ __forceinline__ static Pair from_ints(int x, int y) {
        return __floats2half2_rn(__int2float_rn(x), __int2float_rn(y));
    }

    __device__ __forceinline__ static Pair mul(Pair a, Pair b) { return __hmul2(a, b); }

    __device__ __forceinline__ static Pair fma(Pair a, Pair b, Pair c) { return __hfma2(a, b, c); }

    __device__ __forceinline__ static __half low(Pair v) { return __low2half(v); }

    __device__ __forceinline__ static __half high(Pair v) { return __high2half(v); }
};

template <> struct Vec2Traits<__hip_bfloat16> {
    using Pair = __hip_bfloat162;

    __device__ __forceinline__ static uint32_t to_bits(Pair v) {
        const __hip_bfloat162_raw raw = static_cast<__hip_bfloat162_raw>(v);
    ...
    __device__ __forceinline__ static Pair zero() { return __lows2bfloat162(0.f, 0.f); }

    __device__ __forceinline__ static Pair from_floats(float x, float y) { return __lows2bfloat162(x, y); }

    __device__ __forceinline__ static Pair from_ints(int x, int y) {
        return __lows2bfloat162(__int2float_rn(x), __int2float_rn(y));
    }
    ...
    __device__ __forceinline__ static Pair fma(Pair a, Pair b, Pair c) {
        ...
        uint32_t out;
        asm("{ fma.rn.bf16x2 %0, %1, %2, %3; }\n" : "=r"(out) : "r"(to_bits(a)), "r"(to_bits(b)), "r"(to_bits(c)));
        ...
        return __lows2bfloat162(af.x * bf.x + cf.x, af.y * bf.y + cf.y);
#endif
    }

    __device__ __forceinline__ static __hip_bfloat16 low(Pair v) { return __low2bfloat16(v); }

    __device__ __forceinline__ static __hip_bfloat16 high(Pair v) { return __high2bfloat16(v); }
};
...
template <typename OutType,
          ...
          bool kSharedScale,
          bool kFuseLora>
__global__ void svdquant_scaled_mm_w4a4_kernel(const int8_t * __restrict__ act,           // (M, K/2)
                                               ...
                                               const OutType * __restrict__ lora_up,      // (N, R) or tile-packed
                                               const OutType * __restrict__ bias,
    ...
    constexpr int kBlockKBytes     = 64;  // 64 bytes = 128 nibbles = K=128 (2 groups of 64)
    ...
    constexpr int kSvdGroupBytes   = 32;                  // 64 elements = 32 packed bytes
    constexpr int kSvdSteps        = kSvdGroupBytes / 8;  // 4 K-steps of 8 bytes (16 nibbles)
    ...
    // gfx11 INT4 accumulator: v8i (8 ints per lane)
...
#pragma unroll
    ...
    // FP32 output accumulation (per-group flush)
    float fout[kMtiles][kNtiles][8];
#pragma unroll
    for (int i = 0; i < kMtiles; ++i)
#pragma unroll
        for (int j = 0; j < kNtiles; ++j)
#pragma unroll
    ...
    auto load_tile = [&](const int8_t * src, int row0, int rows_total, int kb0, int total_kbytes, uint4 * regs,
                         int per_thread, int chunks_per_row_local) {
...
#pragma unroll
        for (int i = 0; i < per_thread; ++i) {
            ...
            regs[i]        = (grow < rows_total && gk < total_kbytes) ?
                                 *reinterpret_cast<const uint4 *>(src + (int64_t) grow * total_kbytes + gk) :
                                 make_uint4(0, 0, 0, 0);
        }
    };
...
#pragma unroll
        for (int i = 0; i < per_thread; ++i) {
            const int c   = tid * per_thread + i;
            int8_t *  dst = lds + (c / chunks_per_row_local) * kStride + (c % chunks_per_row_local) * 16;
    ...
    for (int kb0 = 0; kb0 < K_half; kb0 += kBlockKBytes) {
        ...
        if (has_next) {
            load_tile(act, cta_m, M, knext, K_half, regs_a, per_thread_a, chunks_per_row);
            load_tile(wgt, cta_n, N, knext, K_half, regs_b, per_thread_b, chunks_per_row);
        }

        // Process 2 groups of 64 elements each (kBlockKBytes = 64 bytes = 128 nibbles)
#pragma unroll
        for (int grp = 0; grp < kBlockKBytes / kSvdGroupBytes; ++grp) {
            const int g = (kb0 / kSvdGroupBytes) + grp;
            if (g >= num_groups) {
                continue;
            }

            // Reset acc for this group
#pragma unroll
            for (int i = 0; i < kMtiles; ++i)
#pragma unroll
                for (int j = 0; j < kNtiles; ++j) {
            ...
            // K loop: 4 steps of 8 bytes (16 nibbles = K=16)
#pragma unroll
            for (int s = 0; s < kSvdSteps; ++s) {
                const int kbyte = grp * kSvdGroupBytes + s * 8;

                // Load A fragments
                comfy::svdquant::v2i af[kMtiles];
#pragma unroll
                for (int i = 0; i < kMtiles; ++i) {
                    af[i] = comfy::svdquant::load_int4_frag(smem_A, warp_m * kMperWarp + i * 16 + row, kbyte, kStride);
                }

                // Load B fragments
                comfy::svdquant::v2i bf[kNtiles];
#pragma unroll
                for (int j = 0; j < kNtiles; ++j) {
                    bf[j] = comfy::svdquant::load_int4_frag(smem_B, warp_n * kNperWarp + j * 16 + row, kbyte, kStride);
                }
...
#pragma unroll
                    for (int j = 0; j < kNtiles; ++j) {
                        if constexpr (kActUnsigned) {
                            acc[i][j] = comfy::svdquant::wmma_int4_16x16x16_ua(af[i], bf[j], acc[i][j]);
                        } else {
                            acc[i][j] = comfy::svdquant::wmma_int4_16x16x16(af[i], bf[j], acc[i][j]);
                        }
                    }
            }

            // Flush group: rescale int32 acc into fp32 fout
...
#pragma unroll
            for (int j = 0; j < kNtiles; ++j) {
                ...
                const float ws = (col < N) ? load_scale<OutType>(&wscales[g * N + col]) : 0.f;
#pragma unroll
                for (int i = 0; i < kMtiles; ++i) {
#pragma unroll
                    for (int e = 0; e < 8; ++e) {
                        ...
                        const float as = load_scale<OutType>(&ascales[g * M + r]);
    ...
    // ---- Store output with LoRA-up and bias ----
    const int col_lane = lane % 16;
...
#pragma unroll
            for (int e = 0; e < 8; ++e) {
                ...
                if constexpr (kFuseLora) {
                    for (int t = 0; t < R; ++t) {
                        v += load_scale<OutType>(&lora_act_in[r * R + t]) * load_scale<OutType>(&lora_up[col * R + t]);
                    }
                }
                if (bias != nullptr) {
                    v += load_scale<OutType>(&bias[col]);
                }
                out[r * N + col] = fp32_to_out<OutType>(v);
            }
        }
    }
}

// template <typename OutType, bool kActUnsigned, bool kTilePacked, bool kFastAccum, bool kSharedScale,
//           bool kFuseLora>
...
//             int8_t smem_WS[kSharedScale ? kStages : 1][kBlockN * sizeof(OutType)];
...
//                                         else
//                                         {
//                                                 src = wgt + n_global * K_half + g * kBlockKBytes +
//                                                       half * 16;
//                                         }
...
//                                 else
//                                 {
//                                         // Out-of-bounds rows: pad with zeros (use regular 16-byte
//                                         // zero store).
...
//                         }
//                         as_row0_arr[mi] =
//                             (row0_m < M) ? load_scale<OutType>(&ascales[g * M + row0_m]) : 0.f;
//                         as_row1_arr[mi] =
//                             (row1_m < M) ? load_scale<OutType>(&ascales[g * M + row1_m]) : 0.f;
//                 }
//
...
//                                 const OutType* ws_base_g =
//                                     reinterpret_cast<const OutType*>(&smem_WS[cur_stage][0]);
//                                 if (col1 < N)
//                                 {
...
//                                 {
//                                         ws_regs[cc][0] =
//                                             (col0 < N) ? load_scale<OutType>(&ws_base_g[n0]) : 0.f;
//                                         ws_regs[cc][1] = 0.f;
//                                 }
//                         }
//                         else
...
//                                         const int n0 = col0 - cta_n;
//                                         const OutType* ws_base_g =
//                                             &wscales[(cta_n_tile * num_groups + g) * kBlockN];
//                                         if (col1 < N)
//                                         {
//                                                 const float2 ws =
//                                                     load_scale2<OutType>(&ws_base_g[n0]);
//                                                 ws_regs[cc][0] = ws.x;
//                                                 ws_regs[cc][1] = ws.y;
//                                         }
//                                         else
//                                         {
//                                                 ws_regs[cc][0] =
//                                                     (col0 < N) ? load_scale<OutType>(&ws_base_g[n0])
//                                                                : 0.f;
//                                                 ws_regs[cc][1] = 0.f;
//                                         }
//                                 }
...
//                                         {
//                                                 const float2 ws =
//                                                     load_scale2<OutType>(&ws_base_g[col0]);
//                                                 ws_regs[cc][0] = ws.x;
//                                                 ws_regs[cc][1] = ws.y;
//                                         }
//                                         else
...
//                                                 ws_regs[cc][0] =
//                                                     (col0 < N)
//                                                         ? load_scale<OutType>(&ws_base_g[col0])
//                                                         : 0.f;
//                                                 ws_regs[cc][1] = 0.f;
//                                         }
//                                 }
...
//                                         // columns to a packed 16-bit pair and accumulate with
//                                         // h*fma2.
//                                         const Pair ws_pair = Vec2::from_floats(ws_col0, ws_col1);
//                                         const Pair row0_scale = Vec2::mul(
//                                             Vec2::from_floats(as_row0_arr[mi], as_row0_arr[mi]),
//                                             ws_pair);
//                                         const Pair row1_scale = Vec2::mul(
//                                             Vec2::from_floats(as_row1_arr[mi], as_row1_arr[mi]),
//                                             ws_pair);
//                                         out_h[mi][c][0] =
//                                             Vec2::fma(Vec2::from_ints(d_reg[0], d_reg[1]),
//                                                       row0_scale, out_h[mi][c][0]);
//                                         out_h[mi][c][1] =
//                                             Vec2::fma(Vec2::from_ints(d_reg[2], d_reg[3]),
//                                                       row1_scale, out_h[mi][c][1]);
//                                 }
//                                 else
//                                 {
//                                         // fp32 dequant: out_f += cvt(d) * ascale * wscale. Slower
//                                         // than the packed hfma2/bf162 variant but preserves the
//                                         // robust path.
//                                         out_f[mi][c][0] += static_cast<float>(d_reg[0]) *
//                                                            as_row0_arr[mi] * ws_col0;
//                                         out_f[mi][c][1] += static_cast<float>(d_reg[1]) *
//                                                            as_row0_arr[mi] * ws_col1;
//                                         out_f[mi][c][2] += static_cast<float>(d_reg[2]) *
//                                                            as_row1_arr[mi] * ws_col0;
//                                         out_f[mi][c][3] += static_cast<float>(d_reg[3]) *
//                                                            as_row1_arr[mi] * ws_col1;
//                                 }
//                         }
//                 }
//         }
...
//                 {
//                         for (int idx = threadIdx.x; idx < kBlockM * kLoraRankTile;
//                              idx += kThreadsPerBlock)
//                         {
...
//                         }
//
//                         for (int idx = threadIdx.x; idx < kBlockN * kLoraRankTile;
//                              idx += kThreadsPerBlock)
//                         {
...
//                                         if constexpr (kTilePacked)
//                                         {
//                                                 v = lora_up[(cta_n_tile * R + r) * kBlockN +
//                                                             n_local];
//                                         }
//                                         else
//                                         {
...
//
//                         uint32_t a_frag[4];
//                         const OutType* a_addr =
//                             &smem_LoraA[(warp_m * kWarpM + (lane & 15)) * kLoraRankTile +
//                                         ((lane >> 4) * 8)];
...
//                                 uint32_t b_frag4[4];
//                                 const OutType* b_addr =
//                                     &smem_LoraB[(n_off + (lane & 15)) * kLoraRankTile +
//                                                 ((lane >> 4) * 8)];
...
//                                         else
//                                         {
//                                                 mma_m16n8k16_f32<OutType>(out_f[0][b_pair * 2 + 0],
//                                                                           a_frag, b0);
//                                         }
//                                 }
//                                 {
...
//                                         else
//                                         {
//                                                 mma_m16n8k16_f32<OutType>(out_f[0][b_pair * 2 + 1],
//                                                                           a_frag, b1);
//                                         }
//                                 }
//                         }
...
//                                         float v = load_scale<OutType>(&base) +
//                                                   (kFuseLora ? out_f[mi][c][0] : 0.f) +
...
//                         {
//                                 const float bias0 =
//                                     (bias && col0 < N) ? load_scale<OutType>(&bias[col0]) : 0.f;
//                                 const float bias1 =
//                                     (bias && col1 < N) ? load_scale<OutType>(&bias[col1]) : 0.f;
//                                 if (row0_m < M && col0 < N)
//                                         out[row0_m * N + col0] =
//                                             fp32_to_out<OutType>(out_f[mi][c][0] + bias0);
//                                 if (row0_m < M && col1 < N)
//                                         out[row0_m * N + col1] =
//                                             fp32_to_out<OutType>(out_f[mi][c][1] + bias1);
//                                 if (row1_m < M && col0 < N)
//                                         out[row1_m * N + col0] =
//                                             fp32_to_out<OutType>(out_f[mi][c][2] + bias0);
//                                 if (row1_m < M && col1 < N)
//                                         out[row1_m * N + col1] =
//                                             fp32_to_out<OutType>(out_f[mi][c][3] + bias1);
//                         }
//                 }
//         }
// }
...
extern "C" {
void launch_svdquant_scaled_mm_w4a4_kernel(const void * act,
                                           const void * wgt,
                                           const void * ascales,
                                           const void * wscales,
                                           ...
                                           int          fuse_lora,
                                           hipStream_t  stream) {
    if (K % comfy::svdquant::kGroupSize != 0) {
        return;
    }
...
#define LAUNCH_GEMM(OutType, Unsigned, TilePacked, FastAccum, SharedScale, FuseLora)                    \
    svdquant_scaled_mm_w4a4_kernel<OutType, Unsigned, TilePacked, FastAccum, SharedScale, FuseLora>     \
        <<<grid, block, 0, stream>>>(                                                                   \
            reinterpret_cast<const int8_t *>(act), reinterpret_cast<const int8_t *>(wgt),               \
            reinterpret_cast<const OutType *>(ascales), reinterpret_cast<const OutType *>(wscales),     \
            reinterpret_cast<const OutType *>(lora_act_in), reinterpret_cast<const OutType *>(lora_up), \
            reinterpret_cast<const OutType *>(bias), reinterpret_cast<OutType *>(out), M, N, K, R)

#define DISPATCH_SHARED_SCALE(OutType, Unsigned, TilePacked, FastAccum)              \
    do {                                                                             \
        if (shared_scale) {                                                          \
            if (fuse_lora)                                                           \
                LAUNCH_GEMM(OutType, Unsigned, TilePacked, FastAccum, true, true);   \
            else                                                                     \
                LAUNCH_GEMM(OutType, Unsigned, TilePacked, FastAccum, true, false);  \
        } else {                                                                     \
            if (fuse_lora)                                                           \
                LAUNCH_GEMM(OutType, Unsigned, TilePacked, FastAccum, false, true);  \
            else                                                                     \
                LAUNCH_GEMM(OutType, Unsigned, TilePacked, FastAccum, false, false); \
        }                                                                            \
    } while (0)
    ...
    if (out_dtype_code == 2 /* bf16 */) {
        if (tile_packed) {
            if (act_unsigned) {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, true, true, true);
                } else {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, true, true, false);
                }
            } else {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, false, true, true);
                } else {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, false, true, false);
                }
            }
        } else {
            if (act_unsigned) {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, true, false, true);
                } else {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, true, false, false);
                }
            } else {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, false, false, true);
                } else {
                    DISPATCH_SHARED_SCALE(__hip_bfloat16, false, false, false);
                }
            }
        }
        ...
        if (tile_packed) {
            if (act_unsigned) {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__half, true, true, true);
                } else {
                    DISPATCH_SHARED_SCALE(__half, true, true, false);
                }
            } else {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__half, false, true, true);
                } else {
                    DISPATCH_SHARED_SCALE(__half, false, true, false);
                }
            }
        } else {
            if (act_unsigned) {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__half, true, false, true);
                } else {
                    DISPATCH_SHARED_SCALE(__half, true, false, false);
                }
            } else {
                if (fast_accum) {
                    DISPATCH_SHARED_SCALE(__half, false, false, true);
                } else {
                    DISPATCH_SHARED_SCALE(__half, false, false, false);
                }
            }
        }
    }
