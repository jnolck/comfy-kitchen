...
__device__ __forceinline__ int quantize_int4_value(float x, float inv_scale, uint64_t seed,
                                                   int64_t idx)
{
...
template <>
__device__ __forceinline__ float finite_max_for_dtype<float>()
{
        return FLT_MAX;
}
template <>
__device__ __forceinline__ float finite_max_for_dtype<__half>()
{
        return 65504.0f;
}
template <>
__device__ __forceinline__ float finite_max_for_dtype<__hip_bfloat16>()
{
...
inline int fp_dtype_size_bytes(int dtype_code)
{
        switch (dtype_code)
        {
                ...
                case 2:
                        return 2;
                default:
                        throw std::runtime_error("unsupported floating point dtype code");
        }
}
...
template <typename T>
__device__ __forceinline__ void convrot_fht_stage64_typed(const T* __restrict__ src,
                                                          T* __restrict__ dst, int lane, int stride)
{
...
template <typename T>
__device__ __forceinline__ void convrot_fht_stage64_store_typed(const T* __restrict__ src,
                                                                T* __restrict__ output, int lane,
                                                                int stride)
{
        const int base = (lane % stride) + (lane / stride) * (4 * stride);
        const float x0 = to_float(src[base]);
...
template <typename T>
__device__ __forceinline__ float convrot_fht_stage64_store_absmax_typed(const T* __restrict__ src,
...
template <int S>
__device__ __forceinline__ void convrot_fht_stage64_float(const float* __restrict__ src,
...
__device__ __forceinline__ float convrot_fht_stage64_store_absmax_float(
    const float* __restrict__ src, float* __restrict__ output, int lane)
{
        const int base = (lane % S) + (lane / S) * (4 * S);
        const float x0 = src[base];
        const float x1 = src[base + S];
...
__device__ __forceinline__ __hip_bfloat16 bfloat16_fht_scale(__hip_bfloat16 v)
{
        return __hmul(v, __float2bfloat16(0.5f));
}

template <typename T>
__device__ __forceinline__ void convrot_h4_store_typed(T x0, T x1, T x2, T x3,
                                                       T* __restrict__ output, int base)
{
...
__device__ __forceinline__ void convrot_h4_store_typed(__half x0, __half x1, __half x2, __half x3,
                                                       __half* __restrict__ output, int base)
{
                                                       ...
                                                       int base)
{
        ...
        const __hip_bfloat16 x23 = __hadd(x2, x3);
        const __hip_bfloat16 x2m3 = __hsub(x2, x3);
...
template <typename T>
__device__ __forceinline__ void dequant_int4_h4_store_typed(int q0, int q1, int q2, int q3,
                                                            float scale, T* __restrict__ output,
                                                            int base)
{
        ...
        output[base + 3] =
            from_float<T>(static_cast<float>(-q0 + q1 + q2 + q3) * first_stage_scale);
}
                                                            ...
                                                            __half* __restrict__ output, int base)
{
        ...
        const __half2 y01 = __hmul2(__floats2half2_rn(static_cast<float>(q0 + q1 + q2 - q3),
                                                      static_cast<float>(q0 + q1 - q2 + q3)),
                                    s);
        const __half2 y23 = __hmul2(__floats2half2_rn(static_cast<float>(q0 - q1 + q2 + q3),
                                                      static_cast<float>(-q0 + q1 + q2 + q3)),
                                    s);
...
__device__ __forceinline__ void convrot_fht_stage64_typed(const __half* __restrict__ src,
                                                          ...
                                                          int lane, int stride)
{
        ...
        const __hip_bfloat16 x1 = src[base + stride];
        const __hip_bfloat16 x2 = src[base + 2 * stride];
        const __hip_bfloat16 x3 = src[base + 3 * stride];
        const __hip_bfloat16 x01 = __hadd(x0, x1);
        const __hip_bfloat16 x0m1 = __hsub(x0, x1);
        const __hip_bfloat16 x23 = __hadd(x2, x3);
        const __hip_bfloat16 x2m3 = __hsub(x2, x3);
...
template <typename T>
__device__ __forceinline__ void convrot_fht_stage64_vec2_typed(const T* __restrict__ src,
                                                               T* __restrict__ dst, int lane,
                                                               int stride)
{
        convrot_fht_stage64_typed(src, dst, lane, stride);
}

__device__ __forceinline__ void convrot_fht_stage64_vec2_typed(const __half* __restrict__ src,
                                                               __half* __restrict__ dst, int lane,
                                                               int stride)
{
        if (lane & 1)
...
__device__ __forceinline__ void convrot_fht_stage64_vec2_typed(
    const __hip_bfloat16* __restrict__ src, __hip_bfloat16* __restrict__ dst, int lane, int stride)
{
        if (lane & 1)
        {
                return;
        ...
        *reinterpret_cast<__hip_bfloat162*>(dst + base + 2 * stride) =
            __hmul2(__hadd2(x0m1, x23), s);
        *reinterpret_cast<__hip_bfloat162*>(dst + base + 3 * stride) =
            __hmul2(__hsub2(x23, x0m1), s);
}

__device__ __forceinline__ void convrot_fht_stage64_store_typed(const __half* __restrict__ src,
...
template <typename T>
__device__ __forceinline__ void convrot_fht_stage64_store_final_typed(const T* __restrict__ src,
                                                                      T* __restrict__ output,
                                                                      int lane)
{
...
__device__ __forceinline__ void convrot_fht_stage64_store_final_typed(
    const __half* __restrict__ src, __half* __restrict__ output, int lane)
{
        if (lane >= 32)
        {
                return;
    ...
    const __hip_bfloat16* __restrict__ src, __hip_bfloat16* __restrict__ output, int lane)
{
        ...
        *reinterpret_cast<__hip_bfloat162*>(output + col + 192) = __hmul2(__hsub2(x23, x0m1), s);
}

__device__ __forceinline__ float convrot_fht_stage64_store_absmax_typed(
    const __half* __restrict__ src, __half* __restrict__ output, int lane, int stride)
{
        if (stride == 64)
        {
                if (lane >= 32)
    ...
    int stride)
{
        if (stride == 64)
        {
                ...
                const __hip_bfloat162 y3 = __hmul2(__hsub2(x23, x0m1), s);
                *reinterpret_cast<__hip_bfloat162*>(output + col) = y0;
                *reinterpret_cast<__hip_bfloat162*>(output + col + 64) = y1;
                *reinterpret_cast<__hip_bfloat162*>(output + col + 128) = y2;
                *reinterpret_cast<__hip_bfloat162*>(output + col + 192) = y3;
        ...
        const __hip_bfloat16 x23 = __hadd(x2, x3);
        const __hip_bfloat16 x2m3 = __hsub(x2, x3);
        const __hip_bfloat16 y0 = bfloat16_fht_scale(__hadd(x01, x2m3));
        const __hip_bfloat16 y1 = bfloat16_fht_scale(__hsub(x01, x2m3));
        const __hip_bfloat16 y2 = bfloat16_fht_scale(__hadd(x0m1, x23));
        const __hip_bfloat16 y3 = bfloat16_fht_scale(__hsub(x23, x0m1));
        output[base] = y0;
        output[base + stride] = y1;
        output[base + 2 * stride] = y2;
        output[base + 3 * stride] = y3;
...
template <bool STOCHASTIC>
__device__ __forceinline__ uint8_t quantize_int4_pair_byte(float x0, float x1, float inv_scale,
                                                           uint64_t seed, int64_t idx0)
{
...
__device__ __forceinline__ uint32_t quantize_int4_pack4_word(const T* __restrict__ row_buf, int col,
                                                             float inv_scale, uint64_t seed,
                                                             int64_t row_offset)
{
        const uint32_t b0 = quantize_int4_pair_byte<STOCHASTIC>(
            to_float(row_buf[col]), to_float(row_buf[col + 1]), inv_scale, seed, row_offset + col);
        const uint32_t b1 = quantize_int4_pair_byte<STOCHASTIC>(
            to_float(row_buf[col + 2]), to_float(row_buf[col + 3]), inv_scale, seed,
            row_offset + col + 2);
        const uint32_t b2 = quantize_int4_pair_byte<STOCHASTIC>(
            to_float(row_buf[col + 4]), to_float(row_buf[col + 5]), inv_scale, seed,
            row_offset + col + 4);
        const uint32_t b3 = quantize_int4_pair_byte<STOCHASTIC>(
            to_float(row_buf[col + 6]), to_float(row_buf[col + 7]), inv_scale, seed,
            row_offset + col + 6);
        return b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
}

template <typename T, bool STOCHASTIC>
__device__ __forceinline__ uint32_t quantize_int4_values4_int8_word(const T* __restrict__ row_buf,
                                                                    int col, float inv_scale,
                                                                    uint64_t seed,
                                                                    int64_t row_offset)
{
        const uint32_t q0 = static_cast<uint8_t>(quantize_int4_value<STOCHASTIC>(
            to_float(row_buf[col]), inv_scale, seed, row_offset + col));
        const uint32_t q1 = static_cast<uint8_t>(quantize_int4_value<STOCHASTIC>(
            to_float(row_buf[col + 1]), inv_scale, seed, row_offset + col + 1));
        const uint32_t q2 = static_cast<uint8_t>(quantize_int4_value<STOCHASTIC>(
            to_float(row_buf[col + 2]), inv_scale, seed, row_offset + col + 2));
        const uint32_t q3 = static_cast<uint8_t>(quantize_int4_value<STOCHASTIC>(
            to_float(row_buf[col + 3]), inv_scale, seed, row_offset + col + 3));
        return q0 | (q1 << 8) | (q2 << 16) | (q3 << 24);
}

template <typename InType, int BLOCK_THREADS, bool STOCHASTIC>
__global__ void quantize_int4_rowwise_kernel(const InType* __restrict__ x, int8_t* __restrict__ q,
                                             float* __restrict__ scales, int K, uint64_t seed)
{
        ...
        for (int packed_col = tid; packed_col < K_half; packed_col += BLOCK_THREADS)
        {
                ...
                const int q0 = quantize_int4_value<STOCHASTIC>(to_float(x[row_offset + col0]),
                                                               inv_scale, seed, row_offset + col0);
                const int q1 = quantize_int4_value<STOCHASTIC>(to_float(x[row_offset + col1]),
                                                               inv_scale, seed, row_offset + col1);
...
__global__ void quantize_int4_rowwise_convrot64_kernel(const InType* __restrict__ x,
                                                       ...
                                                       uint64_t seed)
{
        ...
        if constexpr (PACK4)
        {
                static_assert(BLOCK_THREADS == 640 || BLOCK_THREADS == 768 || BLOCK_THREADS == 960,
                              "PACK4 path is specialized for K=15360");
#pragma unroll
                for (int it = 0; it < 60 / kGroupsInFlight; ++it)
                {
                        ...
                        convrot_h4_store_typed(x[x_offset], x[x_offset + 1], x[x_offset + 2],
                                               x[x_offset + 3], group_buf, base);
                        __syncwarp();
                        ...
                        abs_max = fmaxf(abs_max, convrot_fht_stage64_store_absmax_typed(
                                                     group_buf, group_buf, lane, 64));
                }
        }
        else
        {
                ...
                for (int it = 0; it < iters; ++it)
                {
                        ...
                        {
                                abs_max = fmaxf(abs_max, convrot_fht_stage64_store_absmax_typed(
                                                             buf1, row_buf + group_col, lane, 64));
                        }
                        __syncthreads();
                }
        }
        ...
        else if constexpr (PACK4)
        {
                ...
                {
                        q_words[word] = quantize_int4_pack4_word<InType, STOCHASTIC>(
                            row_buf, word * 8, inv_scale, seed, row_offset);
                }
        }
        else
        {
...
template <typename InType, int BLOCK_THREADS, bool STOCHASTIC>
__global__ void quantize_int4_rowwise_convrot64_to_int8_float_kernel(const InType* __restrict__ x,
                                                                     int8_t* __restrict__ q,
                                                                     ...
                                                                     int K, uint64_t seed)
{
        ...
        for (int it = 0; it < iters; ++it)
        {
                ...
                if (active)
                {
                        abs_max = fmaxf(abs_max, convrot_fht_stage64_store_absmax_float<64>(
        ...
        for (int col = tid; col < K; col += BLOCK_THREADS)
        {
                ...
                quantized = fminf(static_cast<float>(kInt4Max),
                                  fmaxf(static_cast<float>(-kInt4Max), quantized));
                q[idx] = static_cast<int8_t>(quantized);
        }
}

template <int GROUP_SIZE, typename InType, int BLOCK_THREADS, bool STOCHASTIC>
__global__ void quantize_int4_rowwise_convrot_small_kernel(const InType* __restrict__ x,
                                                           int8_t* __restrict__ q,
                                                           float* __restrict__ scales, int K,
                                                           uint64_t seed)
{
        static_assert(GROUP_SIZE == 16 || GROUP_SIZE == 64,
                      "small ConvRot int4 kernel supports group sizes 16 and 64");
        ...
        for (int it = 0; it < iters; ++it)
        {
                ...
                else
                {
                        ...
                        {
                                abs_max = fmaxf(abs_max, convrot_fht_stage64_store_absmax_typed(
                                                             buf0, row_buf + group_col, lane, 16));
                        }
                }
                __syncthreads();
        }
...
template <int GROUP_SIZE, int GROUPS_PER_BLOCK, typename OutputType, bool CHECK_BOUNDS,
          bool SCALE_PER_ROW>
__global__ void dequantize_int4_convrot_small_kernel(const int8_t* __restrict__ q,
                                                     const float* __restrict__ scales,
                                                     OutputType* __restrict__ output, int K,
                                                     int scale_size)
{
        static_assert(GROUP_SIZE == 16 || GROUP_SIZE == 64,
                      "small ConvRot int4 dequant kernel supports group sizes 16 and 64");
        constexpr int kGroupThreads = GROUP_SIZE / 4;
        extern __shared__ __align__(16) unsigned char smem_raw[];
        ...
        dequant_int4_h4_store_typed(unpack_int4_nibble(packed0), unpack_int4_nibble(packed0 >> 4),
                                    unpack_int4_nibble(packed1), unpack_int4_nibble(packed1 >> 4),
                                    scale, buf1, base);
        __syncthreads();
        ...
        if constexpr (GROUP_SIZE == 16)
        {
                if (active)
                {
                        convrot_fht_stage64_store_typed(buf1, output + row_offset + group_col, lane,
                                                        4);
                }
        }
        else
        {
                ...
                if (active)
                {
                        convrot_fht_stage64_store_typed(buf0, output + row_offset + group_col, lane,
                                                        16);
                }
        }
}
...
__global__ void dequantize_int4_convrot64_kernel(const int8_t* __restrict__ q,
                                                 const float* __restrict__ scales,
                                                 OutputType* __restrict__ output, int K,
                                                 int scale_size)
{
        ...
        const int64_t q_offset =
            (static_cast<int64_t>(row) * (K >> 1)) + group_col_half + (base >> 1);
        uint32_t packed = 0;
        if constexpr (CHECK_BOUNDS)
        {
                ...
                {
                        packed =
                            static_cast<uint32_t>(*reinterpret_cast<const uint16_t*>(q + q_offset));
                }
        }
        else
        {
        ...
        if constexpr (CHECK_BOUNDS)
        {
                if (active)
                {
                        convrot_fht_stage64_store_final_typed(buf1, output + row_offset + group_col,
                                                              lane);
                }
        }
        else
...
template <int GROUPS_PER_BLOCK, typename OutputType, bool CHECK_BOUNDS, bool SCALE_PER_ROW>
__global__ void dequantize_int4_convrot64_warp32_kernel(const int8_t* __restrict__ q,
                                                        const float* __restrict__ scales,
                                                        OutputType* __restrict__ output, int K,
                                                        int scale_size)
{
        ...
        dequant_int4_h4_store_typed(unpack_int4_nibble(packed), unpack_int4_nibble(packed >> 4),
                                    unpack_int4_nibble(packed >> 8),
                                    unpack_int4_nibble(packed >> 12), scale, buf1, base);
        dequant_int4_h4_store_typed(unpack_int4_nibble(packed >> 16),
                                    unpack_int4_nibble(packed >> 20),
                                    unpack_int4_nibble(packed >> 24),
                                    unpack_int4_nibble(packed >> 28), scale, buf1, base + 4);
        __syncwarp();
...
__global__ void int4_weight_int8_act_gemv_dequant_warp_kernel(
    ...
    const float* __restrict__ x_scales, const float* __restrict__ weight_scales,
    const BiasType* __restrict__ bias, OutputType* __restrict__ output, int M, int N, int K,
...
__global__ void dequantize_int4_weight_int8_act_chunk_kernel(
    ...
    OutputType* __restrict__ output, int64_t total, int output_stride, int chunk_cols,
    int col_offset, int weight_scale_size, bool has_bias)
{
...
__global__ void unpack_int4_to_int8_vec8_kernel(const int8_t* __restrict__ input,
                                                int8_t* __restrict__ output, int64_t total_packed)
{
...
extern "C"
{
                                                 ...
                                                 bool stochastic, uint64_t seed, hipStream_t stream)
        {
                ...
                if (input_dtype_code == 2)
                {
                        if (stochastic)
                        {
                                quantize_int4_rowwise_kernel<__hip_bfloat16, kThreads, true>
                                    <<<grid, block, 0, stream>>>(
                                        reinterpret_cast<const __hip_bfloat16*>(input),
                                        reinterpret_cast<int8_t*>(output),
                                        reinterpret_cast<float*>(scales), static_cast<int>(K),
                                        seed);
                        }
                        else
                        {
                                quantize_int4_rowwise_kernel<__hip_bfloat16, kThreads, false>
                                    <<<grid, block, 0, stream>>>(
                                        reinterpret_cast<const __hip_bfloat16*>(input),
                                        reinterpret_cast<int8_t*>(output),
                                        reinterpret_cast<float*>(scales), static_cast<int>(K),
                ...
                if (err != hipSuccess)
                {
                        throw std::runtime_error(
                            std::string("CUDA INT4 rowwise quantization failed: ") +
                            hipGetErrorString(err));
                }
        }

        void launch_quantize_int4_rowwise_convrot64_kernel(const void* input, void* output,
                                                           void* scales, int64_t M, int64_t K,
                                                           int group_size, int input_dtype_code,
                                                           bool stochastic, uint64_t seed,
                                                           hipStream_t stream)
        {
                if ((group_size != 16 && group_size != 64 && group_size != kConvRotGroup) ||
                    K % group_size != 0)
                        return;
                ...
                auto launch =
                    [&](auto kernel, auto typed_input, int block_threads, int scratch_buffers)
                {
                ...
                auto launch_small = [&](auto kernel, auto typed_input, int small_group_size,
                                        int block_threads_small_group)
                {
                        using TypedInputPtr = decltype(typed_input);
                        using TypedInput = std::remove_cv_t<std::remove_pointer_t<TypedInputPtr>>;
                        const int groups_in_flight =
                            block_threads_small_group / (small_group_size / 4);
                        const size_t smem_bytes =
                ...
                auto check_launch = []
                {
                        ...
                        {
                                throw std::runtime_error(
                                    std::string("CUDA INT4 rowwise convrot quantization failed: ") +
                                    hipGetErrorString(err));
                        }
                };
                if (group_size == 16)
                {
                        ...
                        if (input_dtype_code == 2)
                        {
                                auto typed_input = reinterpret_cast<const __hip_bfloat16*>(input);
                                if (use_wide_small_group)
                                {
                                        if (stochastic)
                                        {
                                                launch_small(
                                                    quantize_int4_rowwise_convrot_small_kernel<
                                                        16, __hip_bfloat16, 512, true>,
                                                    typed_input, 16, 512);
                                        }
                                        else
                                        {
                                ...
                                else if (stochastic)
                                {
                                        launch_small(quantize_int4_rowwise_convrot_small_kernel<
                                                         16, __hip_bfloat16, 128, true>,
                                                     typed_input, 16, 128);
                                }
                                else
                                {
                                        launch_small(quantize_int4_rowwise_convrot_small_kernel<
                                                         16, __hip_bfloat16, 128, false>,
                                                     typed_input, 16, 128);
                                }
                        }
                        else if (input_dtype_code == 1)
                        {
                                ...
                                {
                                        launch_small(
                                            quantize_int4_rowwise_convrot_small_kernel<16, __half,
                                                                                       128, true>,
                                            typed_input, 16, 128);
                                }
                                else
                        ...
                        else if (input_dtype_code == 0)
                        {
                                ...
                                {
                                        launch_small(
                                            quantize_int4_rowwise_convrot_small_kernel<16, float,
                                                                                       128, true>,
                                            typed_input, 16, 128);
                                }
                                else
                ...
                if (group_size == 64)
                {
                        ...
                        if (input_dtype_code == 2)
                        {
                                ...
                                {
                                        launch_small(quantize_int4_rowwise_convrot_small_kernel<
                                                         64, __hip_bfloat16, 128, true>,
                                                     typed_input, 64, 128);
                                }
                                else
                                {
                                        launch_small(quantize_int4_rowwise_convrot_small_kernel<
                                                         64, __hip_bfloat16, 128, false>,
                                                     typed_input, 64, 128);
                                }
                        }
                        else if (input_dtype_code == 1)
                        {
                                ...
                                {
                                        launch_small(
                                            quantize_int4_rowwise_convrot_small_kernel<64, __half,
                                                                                       128, true>,
                                            typed_input, 64, 128);
                                }
                                else
                        ...
                        else if (input_dtype_code == 0)
                        {
                                ...
                                {
                                        launch_small(
                                            quantize_int4_rowwise_convrot_small_kernel<64, float,
                                                                                       128, true>,
                                            typed_input, 64, 128);
                                }
                                else
                ...
                if (input_dtype_code == 2)
                {
                        ...
                        if (M != 1 && K <= 4096)
                        {
                                ...
                                else
                                {
                                        launch(
                                            quantize_int4_rowwise_convrot64_kernel<
                                                __hip_bfloat16, block_threads_small, false, false>,
                                            typed_input, block_threads_small, 2);
                                }
                        }
                        else if (M == 1)
                        {
                                ...
                                else
                                {
                                        launch(
                                            quantize_int4_rowwise_convrot64_kernel<
                                                __hip_bfloat16, block_threads_single, false, false>,
                                            typed_input, block_threads_single, 2);
                                }
                        }
                        else if (K == 15360)
                        {
                                if (stochastic)
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __hip_bfloat16, block_threads_15360, true, true>,
                ...
                else if (input_dtype_code == 1)
                {
                        ...
                        if (M != 1 && K <= 4096)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_small, false, true>,
                                               typed_input, block_threads_small, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_small, false, false>,
                                               typed_input, block_threads_small, 2);
                                }
                        }
                        else if (M == 1)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_single, false, true>,
                                               typed_input, block_threads_single, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_single, false, false>,
                                               typed_input, block_threads_single, 2);
                                }
                        }
                        else if (K == 15360)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_15360, true, true>,
                                               typed_input, block_threads_15360, 1);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_15360, true, false>,
                                               typed_input, block_threads_15360, 1);
                                }
                        }
                        else
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_multi, false, true>,
                                               typed_input, block_threads_multi, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_multi, false, false>,
                                               typed_input, block_threads_multi, 2);
                                }
                        }
                }
                else if (input_dtype_code == 0)
                {
                        ...
                        if (M != 1 && K <= 4096)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_small, false, true>,
                                               typed_input, block_threads_small, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_small, false, false>,
                                               typed_input, block_threads_small, 2);
                                }
                        }
                        else if (M == 1)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_single, false, true>,
                                               typed_input, block_threads_single, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_single, false, false>,
                                               typed_input, block_threads_single, 2);
                                }
                        }
                        else if (K == 15360)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_15360, true, true>,
                                               typed_input, block_threads_15360, 1);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_15360, true, false>,
                                               typed_input, block_threads_15360, 1);
                                }
                        }
                        else
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_multi, false, true>,
                                               typed_input, block_threads_multi, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_multi, false, false>,
                                               typed_input, block_threads_multi, 2);
                                }
                        }
                }
        ...
        void launch_quantize_int4_rowwise_convrot64_to_int8_kernel(
            const void* input, void* output, void* scales, int64_t M, int64_t K, int group_size,
            int input_dtype_code, bool stochastic, uint64_t seed, hipStream_t stream)
        {
                ...
                if (input_dtype_code == 2)
                {
                        ...
                        if (K <= 4096)
                        {
                                ...
                                {
                                        launch_float(
                                            quantize_int4_rowwise_convrot64_to_int8_float_kernel<
                                                __hip_bfloat16, block_threads_multi, true>,
                                            typed_input, block_threads_multi);
                                }
                                else
                                {
                                        launch_float(
                ...
                else if (input_dtype_code == 1)
                {
                        ...
                        if (K <= 4096)
                        {
                                if (stochastic)
                                {
                                        launch_float(
                                            quantize_int4_rowwise_convrot64_to_int8_float_kernel<
                                                __half, block_threads_multi, true>,
                                            typed_input, block_threads_multi);
                                }
                                else
                                {
                                        launch_float(
                                            quantize_int4_rowwise_convrot64_to_int8_float_kernel<
                                                __half, block_threads_multi, false>,
                                            typed_input, block_threads_multi);
                                }
                        }
                        else if (M == 1)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_single, false, true, true>,
                                               typed_input, block_threads_single, 2);
                                }
                                else
                                {
                                        launch(
                                            quantize_int4_rowwise_convrot64_kernel<
                                                __half, block_threads_single, false, false, true>,
                                            typed_input, block_threads_single, 2);
                                }
                        }
                        else if (K == 15360)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_15360, true, true, true>,
                                               typed_input, block_threads_15360, 1);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_15360, true, false, true>,
                                               typed_input, block_threads_15360, 1);
                                }
                        }
                        else
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_multi, false, true, true>,
                                               typed_input, block_threads_multi, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   __half, block_threads_multi, false, false, true>,
                                               typed_input, block_threads_multi, 2);
                                }
                        }
                }
                else if (input_dtype_code == 0)
                {
                        ...
                        if (K <= 4096)
                        {
                                if (stochastic)
                                {
                                        launch_float(
                                            quantize_int4_rowwise_convrot64_to_int8_float_kernel<
                                                float, block_threads_multi, true>,
                                            typed_input, block_threads_multi);
                                }
                                else
                                {
                                        launch_float(
                                            quantize_int4_rowwise_convrot64_to_int8_float_kernel<
                                                float, block_threads_multi, false>,
                                            typed_input, block_threads_multi);
                                }
                        }
                        else if (M == 1)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_single, false, true, true>,
                                               typed_input, block_threads_single, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_single, false, false, true>,
                                               typed_input, block_threads_single, 2);
                                }
                        }
                        else if (K == 15360)
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_15360, true, true, true>,
                                               typed_input, block_threads_15360, 1);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_15360, true, false, true>,
                                               typed_input, block_threads_15360, 1);
                                }
                        }
                        else
                        {
                                ...
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_multi, false, true, true>,
                                               typed_input, block_threads_multi, 2);
                                }
                                else
                                {
                                        launch(quantize_int4_rowwise_convrot64_kernel<
                                                   float, block_threads_multi, false, false, true>,
                                               typed_input, block_threads_multi, 2);
                                }
                        }
                }
                ...
                if (err != hipSuccess)
                {
                        throw std::runtime_error(
                            std::string(
                                "CUDA INT4 rowwise convrot INT8-output quantization failed: ") +
                            hipGetErrorString(err));
                }
        }

        void launch_dequantize_int4_convrot64_kernel(const void* input, const void* scales,
                                                     void* output, int64_t num_rows,
                                                     int64_t num_cols, int64_t scale_size,
                                                     int group_size, int output_dtype_code,
                                                     hipStream_t stream)
        {
                if (num_rows == 0 || num_cols == 0) return;
                if ((group_size != 16 && group_size != 64 && group_size != kConvRotGroup) ||
                    num_cols % group_size != 0)
                        return;
                ...
                auto launch_small_groups = [&](auto group_tag, auto groups_tag)
                {
                        ...
                        const int group_blocks =
                            static_cast<int>((num_cols / small_group_size + groups_per_block - 1) /
                                             groups_per_block);
                        dim3 grid(static_cast<unsigned int>(group_blocks),
                                  static_cast<unsigned int>(num_rows));
                        const bool check_bounds =
                            ((num_cols / small_group_size) % groups_per_block) != 0;
                        ...
                        if (output_dtype_code == 2)
                        {
                                ...
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                                dequantize_int4_convrot_small_kernel<
                                                    small_group_size, groups_per_block,
                                                    __hip_bfloat16, true, true>
                        ...
                        {
                                const size_t smem_bytes =
                                    groups_per_block * 2 * small_group_size * sizeof(__half);
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                                dequantize_int4_convrot_small_kernel<
                                                    small_group_size, groups_per_block, __half,
                                                    true, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                        ...
                        {
                                const size_t smem_bytes =
                                    groups_per_block * 2 * small_group_size * sizeof(float);
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                                dequantize_int4_convrot_small_kernel<
                                                    small_group_size, groups_per_block, float, true,
                                                    true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                ...
                                else
                                {
                                        ...
                                        {
                                                dequantize_int4_convrot_small_kernel<
                                                    small_group_size, groups_per_block, float,
                                                    false, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                ...
                if (group_size == 16)
                {
                        launch_small_groups(std::integral_constant<int, 16>{},
                                            std::integral_constant<int, 32>{});
                        return;
                }
                if (group_size == 64)
                {
                        launch_small_groups(std::integral_constant<int, 64>{},
                ...
                auto launch_groups = [&](auto groups_tag)
                {
                        ...
                        const int group_blocks = static_cast<int>(
                            (num_cols / kConvRotGroup + groups_per_block - 1) / groups_per_block);
                        dim3 grid(static_cast<unsigned int>(group_blocks),
                                  static_cast<unsigned int>(num_rows));
                        const bool check_bounds =
                            ((num_cols / kConvRotGroup) % groups_per_block) != 0;
                        const bool scale_per_row = scale_size != 1;

                        if (output_dtype_code == 2)
                        {
                                const size_t smem_bytes =
                                    groups_per_block * 2 * kConvRotGroup * sizeof(__hip_bfloat16);
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __hip_bfloat16, true, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__hip_bfloat16*>(output),
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __hip_bfloat16, true, false>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__hip_bfloat16*>(output),
                                ...
                                else
                                {
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __hip_bfloat16, false, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__hip_bfloat16*>(output),
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __hip_bfloat16, false, false>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__hip_bfloat16*>(output),
                        ...
                        {
                                const size_t smem_bytes =
                                    groups_per_block * 2 * kConvRotGroup * sizeof(__half);
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                                dequantize_int4_convrot64_kernel<groups_per_block,
                                                                                 __half, true, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __half, true, false>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__half*>(output),
                                ...
                                else
                                {
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __half, false, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__half*>(output),
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, __half, false, false>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__half*>(output),
                        ...
                        {
                                const size_t smem_bytes =
                                    groups_per_block * 2 * kConvRotGroup * sizeof(float);
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                ...
                                else
                                {
                                        ...
                                        {
                                                dequantize_int4_convrot64_kernel<
                                                    groups_per_block, float, false, false>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<float*>(output),
                ...
                auto launch_groups_warp32 = [&](auto groups_tag)
                {
                        ...
                        if (output_dtype_code == 2)
                        {
                                ...
                                if (check_bounds)
                                {
                                        if (scale_per_row)
                                        {
                                                dequantize_int4_convrot64_warp32_kernel<
                                                    groups_per_block, __hip_bfloat16, true, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                        ...
                        else
                        {
                                ...
                                if (check_bounds)
                                {
                                        ...
                                        {
                                                dequantize_int4_convrot64_warp32_kernel<
                                                    groups_per_block, __half, true, true>
                                                    <<<grid, block_threads, smem_bytes, stream>>>(
                                                        reinterpret_cast<const int8_t*>(input),
                                                        reinterpret_cast<const float*>(scales),
                                                        reinterpret_cast<__half*>(output),
        ...
        void launch_int4_linear_kernel(const void* act, const void* weight, const void* x_scales,
                                       ...
                                       int output_dtype_code, int bias_dtype_code,
                                       hipStream_t stream)
        {
                if (K % comfy::svdquant::kGroupSize != 0) return;
...
#define DISPATCH_OUT_BIAS(OutType, BiasType)                                               \
        int4_linear_kernel<OutType, BiasType><<<grid, block, 0, stream>>>(                 \
            reinterpret_cast<const int8_t*>(act), reinterpret_cast<const int8_t*>(weight), \
            reinterpret_cast<const float*>(x_scales),                                      \
            reinterpret_cast<const float*>(weight_scales),                                 \
            reinterpret_cast<const BiasType*>(bias), reinterpret_cast<OutType*>(output),   \
            static_cast<int>(M), static_cast<int>(N), static_cast<int>(K), has_bias)
...
#undef DISPATCH_OUT_BIAS
                                               ...
                                               int64_t K_half, hipStream_t stream)
        {
                ...
                unpack_int4_to_int8_vec8_kernel<<<static_cast<unsigned int>(blocks), block_threads,
                                                  0, stream>>>(
        ...
        void launch_int4_weight_int8_act_gemv_dequant_kernel(
            const void* input, const void* weight, const void* x_scales, const void* weight_scales,
            const void* bias, void* output, int64_t num_rows, int64_t num_cols, int64_t K,
            int64_t weight_scale_size, bool has_bias, int output_dtype_code, int bias_dtype_code,
            hipStream_t stream)
        {
                ...
                {
                        throw std::runtime_error(
                            "int4_weight_int8_act_gemv_dequant requires K divisible by 4");
                }
                ...
                {
                        throw std::runtime_error(
                            "int4_weight_int8_act_gemv_dequant only supports M,N,K <= INT_MAX");
                }
                if (weight_scale_size != 1 && weight_scale_size != num_cols)
                {
                        throw std::runtime_error(
                            "INT4 packed GEMV weight scale must be scalar or per-output-channel");
                }
                ...
                DISPATCH_FP_DTYPE(
                    output_dtype_code, OutputType,
                    [&]
                    {
                            if (!has_bias)
                            {
                                    int4_weight_int8_act_gemv_dequant_warp_kernel<kWarpsPerBlock,
                                                                                  OutputType, float>
                                        <<<grid, kWarpsPerBlock * kThreadsPerWarp, 0, stream>>>(
                                            ...
                                            static_cast<int>(K),
                                            static_cast<int>(weight_scale_size), false);
                                    return;
                            }

                            DISPATCH_FP_DTYPE(
                                bias_dtype_code, BiasType,
                                [&]
                                {
                                        int4_weight_int8_act_gemv_dequant_warp_kernel<
                                            kWarpsPerBlock, OutputType, BiasType>
                                            <<<grid, kWarpsPerBlock * kThreadsPerWarp, 0, stream>>>(
                                                static_cast<const int8_t*>(input),
                                                static_cast<const int8_t*>(weight),
                                                static_cast<const float*>(x_scales),
                                                ...
                                                static_cast<int>(num_cols), static_cast<int>(K),
                                                static_cast<int>(weight_scale_size), true);
                                });
                    });
                ...
                {
                        throw std::runtime_error(
                            std::string("CUDA packed INT4 weight GEMV failed: ") +
                            hipGetErrorString(err));
                }
        }
            ...
            hipStream_t stream)
        {
                ...
                {
                        throw std::runtime_error(
                            "int4_weight_int8_act_gemm_dequant_chunked requires K divisible by 2");
                }
                if (chunk_cols <= 0)
                {
                        throw std::runtime_error(
                            "int4_weight_int8_act_gemm_dequant_chunked requires positive "
                            "chunk_cols");
                }
                if (num_cols > static_cast<int64_t>(std::numeric_limits<int>::max()) ||
                    num_rows > static_cast<int64_t>(std::numeric_limits<int>::max()) ||
                ...
                {
                        throw std::runtime_error(
                            "int4_weight_int8_act_gemm_dequant_chunked only supports M,N,K,chunk "
                            "<= INT_MAX");
                }
                if (weight_scale_size != 1 && weight_scale_size != num_cols)
                {
                        throw std::runtime_error(
                            "INT4 chunked GEMM weight scale must be scalar or per-output-channel");
                }
                ...
                for (int64_t n0 = 0; n0 < num_cols; n0 += chunk_cols)
                {
                        ...
                        const int64_t unpack_blocks =
                            ((total_packed + 7) / 8 + unpack_threads - 1) / unpack_threads;
                        unpack_int4_to_int8_vec8_kernel<<<static_cast<unsigned int>(unpack_blocks),
                                                          unpack_threads, 0, stream>>>(
                            static_cast<const int8_t*>(weight) + n0 * K_half,
                            static_cast<int8_t*>(weight_workspace), total_packed);

                        const float* chunk_weight_scales =
                            static_cast<const float*>(weight_scales) +
                            (weight_scale_size == 1 ? 0 : n0);
                        const void* chunk_bias = has_bias
                        ...
                        launch_cublas_gemm_int8_kernel(input, weight_workspace, acc_workspace,
                                                       ...
                                                       cublas_workspace_size, stream);

                        const int64_t total = num_rows * cols;
                        const int64_t dequant_blocks =
                            (total + dequant_threads - 1) / dequant_threads;
                        DISPATCH_FP_DTYPE(
                            output_dtype_code, OutputType,
                            [&]
                            {
                                    if (!has_bias)
                                    {
                                            dequantize_int4_weight_int8_act_chunk_kernel<OutputType,
                                                                                         float>
                                                <<<static_cast<unsigned int>(dequant_blocks),
                                                   dequant_threads, 0, stream>>>(
                                                    static_cast<const int32_t*>(acc_workspace),
                                                    static_cast<const float*>(x_scales),
                                                    static_cast<const float*>(weight_scales),
                                                    nullptr, static_cast<OutputType*>(output),
                                                    total, static_cast<int>(num_cols),
                                                    static_cast<int>(cols), static_cast<int>(n0),
                                                    static_cast<int>(weight_scale_size), false);
                                            return;
                                    }
                                    ...
                                    DISPATCH_FP_DTYPE(
                                        ...
                                        [&]
                                        {
                                                dequantize_int4_weight_int8_act_chunk_kernel<
                                                    OutputType, BiasType>
                                                    <<<static_cast<unsigned int>(dequant_blocks),
                                                       dequant_threads, 0, stream>>>(
                                                        static_cast<const int32_t*>(acc_workspace),
