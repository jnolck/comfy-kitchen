...
__global__ void apply_rope_kernel(const InputType* xq, const InputType* xk, const FreqsType* freqs,
                                  InputType* xq_out, InputType* xk_out, int64_t batch, int64_t dim1,
                                  int64_t dim2, int64_t head_dim, int64_t freqs_batch,
                                  int64_t freqs_dim1, int64_t freqs_dim2, int64_t stride_x_batch,
                                  int64_t stride_x_dim1, int64_t stride_x_dim2,
                                  int64_t stride_x_dim, int64_t stride_freqs_batch,
                                  ...
                                  int64_t stride_freqs_pair, bool split_half)
{
        ...
        auto load_freqs_matrix = [&](int64_t pair_idx, ComputeType& f00, ComputeType& f01,
                                     ComputeType& f10, ComputeType& f11)
        {
                const int64_t freqs_base =
                    freqs_batch_idx * stride_freqs_batch + freqs_dim1_idx * stride_freqs_dim1 +
                ...
                if (stride_freqs_pair == 1 && stride_freqs_rot == 2)
                {
                        ...
                        {
                                // fp16/bf16: single float2 load, reinterpret as 4 half elements (4
                                // x 2 bytes = 8 bytes)
                                const float2 vec =
        ...
        auto load_pairs = [&](const InputType* x_in, ComputeType& x_0, ComputeType& x_1,
                              ComputeType& x_2, ComputeType& x_3)
        {
                if (split_half)
                {
                        if (stride_x_dim == 1)
                        {
                                // First half: pair_idx_0 and pair_idx_1 are adjacent — one float
                                // load (2 halfs)
                                union
                                {
                                        float vec;
                        ...
                        else
                        {
                                ...
                                if (process_pair_1)
                                {
                                        ...
                                        x_3 = convert_via_float<ComputeType>(x_in[calc_offset(
                                            (pair_idx_1 + n_pairs) * stride_x_dim)]);
                                }
                        }
                }
                else
                {
                        ...
                        else
                        {
                                ...
                                {
                                        x_2 = convert_via_float<ComputeType>(
                                            x_in[off0 + 2 * stride_x_dim]);
                                        x_3 = convert_via_float<ComputeType>(
                                            x_in[off0 + 3 * stride_x_dim]);
                                }
                        }
                }
        };
        ...
        // strided.
        auto store_pairs = [&](InputType* x_out, ComputeType x_0, ComputeType x_1, ComputeType x_2,
                               ComputeType x_3)
        {
                if (split_half)
                {
                        if (stride_x_dim == 1)
                        {
                                ...
                                *reinterpret_cast<float*>(
                                    &x_out[calc_offset(pair_idx_0 + n_pairs)]) = hi.vec;
                        }
                        else
                        {
                ...
                else
                {
                        ...
                        else
                        {
                                ...
                                if (process_pair_1)
                                {
                                        x_out[off0 + 2 * stride_x_dim] =
                                            convert_via_float<InputType>(x_2);
                                        x_out[off0 + 3 * stride_x_dim] =
                                            convert_via_float<InputType>(x_3);
                                }
                        }
                }
...
template <typename InputType, typename FreqsType>
void apply_rope_launcher(const InputType* xq, const InputType* xk, const FreqsType* freqs,
                         InputType* xq_out, InputType* xk_out, int64_t batch, int64_t dim1,
                         ...
                         int64_t stride_freqs_dim1, int64_t stride_freqs_dim2,
                         int64_t stride_freqs_dim, int64_t stride_freqs_rot,
                         int64_t stride_freqs_pair, bool split_half, hipStream_t stream)
{
        const int64_t n_pairs = head_dim / 2;
        const int64_t pairs_per_thread = 2;
        const int64_t n_pair_groups = (n_pairs + pairs_per_thread - 1) / pairs_per_thread;
        ...
        apply_rope_kernel<InputType, FreqsType><<<num_blocks, block_size, 0, stream>>>(
            xq, xk, freqs, xq_out, xk_out, batch, dim1, dim2, head_dim, freqs_batch, freqs_dim1,
            freqs_dim2, stride_x_batch, stride_x_dim1, stride_x_dim2, stride_x_dim,
...
extern "C"
{
        void launch_apply_rope_kernel_old(
