...
extern "C" void launch_dequant_int4_grouped_to_int8(
    ...
    int64_t N, int64_t K, int64_t G, hipStream_t stream)
{
    const int Khalf = K / 2;
...
__device__ __forceinline__ float rq_to_float(__hip_bfloat16 v) { return __bfloat162float(v); }
    ...
    int K, uint64_t seed)
{
    ...
    for (int g = tid; g < groups; g += nthreads) {
        ...
        const uint8_t srel_bits = __hip_cvt_float_to_fp8(srel_f, __HIP_SATFINITE, __HIP_E4M3_FNUZ);
        s_rel[srow_off + g] = srel_bits;
        const float srel_r = __half2float(__hip_cvt_fp8_to_halfraw(srel_bits, __HIP_E4M3_FNUZ));
        float lv[16];
        #pragma unroll
    ...
    int64_t N, int64_t K, int in_dtype_code, bool stochastic, uint64_t seed, hipStream_t stream)
{
    ...
    if (hipDeviceGetAttribute(&max_shmem, hipDeviceAttributeMaxSharedMemoryPerBlock, dev) != hipSuccess)
        return false;
