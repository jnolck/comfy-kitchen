...
__device__ __forceinline__ float load_scale(ScaleT v);
template <>
__device__ __forceinline__ float load_scale<float>(float v)
{
        return v;
}
template <>
...
extern "C" void launch_dequant_int4_grouped_to_int8(const void* qw, const void* s_rel,
                                                    ...
                                                    int64_t K, int64_t G, hipStream_t stream)
{
        ...
        dequant_int4_grouped_to_int8_kernel<float><<<grid, block, 0, stream>>>(
            ...
            static_cast<const float*>(codebook), static_cast<int8_t*>(out), n_vec, Khalf,
            static_cast<int>(K), static_cast<int>(G));
}

// fp8 (e4m3) per-group scale variant; s_rel passed as raw uint8 bits.
extern "C" void launch_dequant_int4_grouped_to_int8_e4m3(const void* qw, const void* s_rel,
                                                         const void* codebook, void* out, int64_t N,
                                                         int64_t K, int64_t G, hipStream_t stream)
{
        const int Khalf = K / 2;
...
extern "C" bool launch_w4a8_codebook_gemm_chunked(
    ...
    int64_t M, int64_t N, int64_t K, int64_t G, int64_t chunk_cols, int out_dtype_code,
    hipStream_t stream)
{
        ...
        for (int64_t n0 = 0; n0 < N; n0 += chunk_cols)
        {
                ...
                launch_dequant_int4_grouped_to_int8_e4m3(
                    ...
                    static_cast<const uint8_t*>(s_rel) + n0 * KG, codebook, workspace, cols, K, G,
                    stream);
...
__device__ __forceinline__ float rq_to_float(float v)
{
        return v;
}
__device__ __forceinline__ float rq_to_float(__half v)
{
        return __half2float(v);
}
__device__ __forceinline__ float rq_to_float(__hip_bfloat16 v)
{
...
#pragma unroll
        for (int o = 16; o > 0; o >>= 1)
                v = fmaxf(v, __shfl_down_sync(0xfffffffful, v, o));
        return v;
}
...
{
        int best = 0;
        float bd = fabsf(x - cb[0]);
#pragma unroll
        for (int j = 1; j < 16; ++j)
        {
                float d = fabsf(x - cb[j]);
                if (d < bd)
                {
                        bd = d;
                        best = j;
...
__global__ void quantize_w4a8_convrot_kernel(const InputType* __restrict__ rotated,  // [N, K]
...
#pragma unroll
                for (int i = 0; i < G; ++i)
                        thr_amax = fmaxf(thr_amax, fabsf(cb[idx[i]] * gs));
        }
...
#pragma unroll
                for (int j = 0; j < 16; ++j)
                        lv[j] = fminf(127.0f, fmaxf(-127.0f, nearbyintf(cb[j] * srel_r)));
                const int64_t base = row_off + static_cast<int64_t>(g) * G;
                int u[G];
#pragma unroll
                for (int i = 0; i < G; ++i)
...
#pragma unroll
                                ...
                                const float thr =
                                    lv[lo] + rq_uniform(base + i, seed) * (lv[lo + 1] - lv[lo]);
...
extern "C" bool launch_quantize_w4a8_convrot(const void* rotated, const void* codebook,
                                             void* packed, void* s_rel, void* s_channel, int64_t N,
                                             int64_t K, int in_dtype_code, bool stochastic,
                                             uint64_t seed, hipStream_t stream)
{
        ...
        if (hipDeviceGetAttribute(&max_shmem, hipDeviceAttributeMaxSharedMemoryPerBlock, dev) !=
            hipSuccess)
                return false;
