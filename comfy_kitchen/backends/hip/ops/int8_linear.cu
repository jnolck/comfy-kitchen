...
template <>
__device__ __forceinline__ float finite_max_for_dtype<half>()
{
        return 65504.0f;
}
template <>
...
template <>
__device__ __forceinline__ __hip_bfloat16 from_float<__hip_bfloat16>(float val)
{
...
template <>
__device__ __forceinline__ float stochastic_sum_to_float<__hip_bfloat16>(float scaled,
                                                                         __hip_bfloat16 rng)
{
        return __bfloat162float(__hadd(__float2bfloat16(scaled), rng));
}
...
template <typename T>
__device__ __forceinline__ void store4_contiguous(T* out, int64_t idx, float x, float y, float z,
                                                  float w)
{
...
template <>
__device__ __forceinline__ void store4_contiguous<float>(float* out, int64_t idx, float x, float y,
                                                         float z, float w)
{
...
template <>
__device__ __forceinline__ void store4_contiguous<half>(half* out, int64_t idx, float x, float y,
                                                        float z, float w)
{
...
template <>
__device__ __forceinline__ void store4_contiguous<__hip_bfloat16>(__hip_bfloat16* out, int64_t idx,
                                                                  float x, float y, float z,
                                                                  float w)
{
        // reinterpret_cast<__hip_bfloat162*>(out)[idx / 2] = __lows2bfloat162(x, y);
        // reinterpret_cast<__hip_bfloat162*>(out)[idx / 2 + 1] = __lows2bfloat162(z, w);
        //  Convert and store directly
        out[idx] = __float2bfloat16(x);
        out[idx + 1] = __float2bfloat16(y);
        out[idx + 2] = __float2bfloat16(z);
        out[idx + 3] = __float2bfloat16(w);
}
...
__device__ __forceinline__ float warp_reduce_max(float v)
{
        ...
        {
                v = fmaxf(v, __shfl_down_sync(0xffffffffull, v, offset));
        }
        return v;
}
                                             ...
                                             int K, uint64_t seed)
{
        ...
        const float scale =
            fmaxf(finite_absmax_for_int8_scale<InputType>(abs_max) * (1.0f / 127.0f), 1.0e-30f);
...
template <typename InputType, int BLOCK_THREADS, bool STOCHASTIC>
__global__ void quantize_int8_rowwise_convrot_kernel(const InputType* __restrict__ x,
...
__global__ void dequantize_int8_linear_kernel(const int32_t* __restrict__ input,
...
__global__ void dequantize_int8_linear_vec4_kernel(const int32_t* __restrict__ input,
                                                   const float* __restrict__ x_scales,
                                                   const float* __restrict__ weight_scales,
                                                   OutputType* __restrict__ output,
                                                   int64_t total_vec4, int N, int weight_scale_size)
{
        ...
        if (weight_scale_size == 1)
        {
                ...
                store4_contiguous(output, idx, static_cast<float>(acc.x) * scale,
        ...
        store4_contiguous(output, idx, static_cast<float>(acc.x) * x_scale * ws.x,
...
__global__ void int8_gemv_dequant_kernel(const int8_t* __restrict__ x,
                                         const int8_t* __restrict__ weight,
                                         const float* __restrict__ x_scales,
                                         const float* __restrict__ weight_scales,
                                         const BiasType* __restrict__ bias,
                                         OutputType* __restrict__ output, int N, int K,
                                         int weight_scale_size, bool has_bias)
{
        constexpr int kWarps = BLOCK_THREADS / kThreadsPerWarp;
        ...
        {
                acc = amd_mixed_dot(x4[k4], w4[k4], acc, false);
        }
...
__global__ void int8_gemv_dequant_warp_kernel(const int8_t* __restrict__ x,
                                              const int8_t* __restrict__ weight,
                                              const float* __restrict__ x_scales,
                                              const float* __restrict__ weight_scales,
                                              const BiasType* __restrict__ bias,
                                              ...
                                              int weight_scale_size, bool has_bias)
{
        ...
        const int* __restrict__ w4 =
            reinterpret_cast<const int*>(weight + static_cast<int64_t>(n) * K);
...
template <typename OutputType>
__global__ void dequantize_int8_simple_kernel(const int8_t* __restrict__ input,
                                              const float* __restrict__ scales,
                                              OutputType* __restrict__ output, int64_t total,
                                              int inner_dim, int scale_mode)
{
        const int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        if (idx >= total)
        {
...
template <typename OutputType>
__global__ void dequantize_int8_simple_vec4_kernel(const int8_t* __restrict__ input,
                                                   const float* __restrict__ scales,
                                                   OutputType* __restrict__ output,
                                                   int64_t total_vec4, int inner_dim_vec4,
                                                   int scale_mode)
{
        const int64_t idx4 = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        if (idx4 >= total_vec4)
        ...
        const int64_t idx = idx4 * 4;
        store4_contiguous(output, idx, static_cast<float>(q4.x) * scale,
                          static_cast<float>(q4.y) * scale, static_cast<float>(q4.z) * scale,
                          static_cast<float>(q4.w) * scale);
}

template <typename OutputType>
__global__ void dequantize_int8_rowwise_vec4_2d_kernel(const int8_t* __restrict__ input,
                                                       const float* __restrict__ scales,
                                                       OutputType* __restrict__ output, int rows,
                                                       int inner_dim_vec4)
{
        const int row = static_cast<int>(blockIdx.x);
...
__global__ void dequantize_int8_convrot_kernel(const int8_t* __restrict__ q,
                                               const float* __restrict__ scales,
...
template <int BLOCK_THREADS, typename OutputType>
__global__ void dequantize_int8_convrot_groups_kernel(const int8_t* __restrict__ q,
                                                      const float* __restrict__ scales,
                                                      OutputType* __restrict__ output, int K,
                                                      int scale_size)
{
        ...
        const int group =
            static_cast<int>(blockIdx.x) * kGroupsPerBlock + threadIdx.x / kConvRotGroup;
...
#pragma unroll
        for (int stage = 0; stage < 4; ++stage)
        {
                ...
                const int d = (i / s) & 3;
                const int base = i - d * s;
                const float v = 0.5f * h4_row_dot(d, src[base], src[base + s], src[base + 2 * s],
                                                  src[base + 3 * s]);
                dst[i] = v;
                __syncthreads();
                float* t = src;
...
template <int S>
__device__ __forceinline__ void convrot_fht_stage64(const float* __restrict__ src,
...
__device__ __forceinline__ void convrot_fht_stage64_store(const float* __restrict__ src,
                                                          OutputType* __restrict__ output, int lane)
{
        const int base = (lane % S) + (lane / S) * (4 * S);
        const float x0 = src[base];
        const float x1 = src[base + S];
...
template <int S, typename OutputType>
__device__ __forceinline__ float convrot_fht_stage64_store_absmax(const float* __restrict__ src,
                                                                  OutputType* __restrict__ output,
                                                                  int lane)
{
        const int base = (lane % S) + (lane / S) * (4 * S);
...
__global__ void dequantize_int8_convrot_groups64_kernel(const int8_t* __restrict__ q,
                                                        const float* __restrict__ scales,
                                                        OutputType* __restrict__ output, int K,
                                                        int scale_size)
{
        ...
        if (active)
        {
                convrot_fht_stage64_store<64, OutputType>(buf1, output + row_offset + group_col,
                                                          lane);
        }
}

template <int GROUPS_PER_BLOCK, typename InputType, typename OutputType>
__global__ void rotate_int8_convrot_groups64_amax_kernel(const InputType* __restrict__ x,
                                                         OutputType* __restrict__ output,
...
template <int GROUPS_PER_BLOCK, typename InputType, typename OutputType>
__global__ void rotate_int8_convrot_groups64_kernel(const InputType* __restrict__ x,
                                                    OutputType* __restrict__ output, int K)
{
        constexpr int kGroupThreads = 64;
        extern __shared__ float smem[];
...
template <typename InputType, int BLOCK_THREADS, bool STOCHASTIC>
__global__ void quantize_int8_rowwise_from_partials_kernel(const InputType* __restrict__ x,
...
__device__ __forceinline__ float load_input_act(const InputType* __restrict__ x, int64_t in_row,
                                                int col, int K)
{
...
__global__ void quantize_int8_rowwise_convrot64_kernel(const InputType* __restrict__ x,
                                                       int8_t* __restrict__ q,
                                                       float* __restrict__ scales, int K,
                                                       uint64_t seed)
{
        ...
        for (int it = 0; it < iters; ++it)
        {
                ...
                if (active)
                {
                        abs_max = fmaxf(abs_max, convrot_fht_stage64_store_absmax<64, float>(
...
extern "C"
{
        void launch_quantize_int8_rowwise_kernel(const void* input, void* output, void* scales,
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    [&]
                    {
                            ...
                            if (num_cols >= 4096 && num_rows != 1)
                            {
                                    if (stochastic)
                                    {
                                            launch(comfy::quantize_int8_rowwise_kernel<InputType,
                                                                                       512, true>,
                                                   512);
                                    }
                                    else
                            ...
                            else
                            {
                                    if (stochastic)
                                    {
                                            launch(comfy::quantize_int8_rowwise_kernel<
                                                       InputType, comfy::kInt8Threads, true>,
                                                   comfy::kInt8Threads);
                                    }
                                    else
                                    {
                                            launch(comfy::quantize_int8_rowwise_kernel<
                                                       InputType, comfy::kInt8Threads, false>,
                                                   comfy::kInt8Threads);
                                    }
                            }
                    });
                                                         ...
                                                         uint64_t seed, hipStream_t stream)
        {
                ...
                const size_t smem_bytes =
                    (static_cast<size_t>(num_cols) + groups_in_flight * 2 * comfy::kConvRotGroup) *
                    sizeof(float);
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    [&]
                    {
                            auto launch = [&](auto kernel)
                            {
                                    ...
                                    {
                                            throw std::runtime_error(
                                                std::string("convrot fused kernel shared memory "
                                                            "request (") +
                                                std::to_string(smem_bytes) +
                                                " bytes) failed: " + hipGetErrorString(attr_err));
                                    }
                                    kernel<<<static_cast<unsigned int>(num_rows), block_threads,
                                             smem_bytes, stream>>>(
                                        static_cast<const InputType*>(input),
                                        static_cast<int8_t*>(output), static_cast<float*>(scales),
                                        static_cast<int>(num_cols), seed);
                            };
                            if (wide)
                            {
                                    if (stochastic)
                                    {
                                            launch(comfy::quantize_int8_rowwise_convrot_kernel<
                                                   InputType, 1024, true>);
                                    }
                                    else
                                    {
                                            launch(comfy::quantize_int8_rowwise_convrot_kernel<
                                                   InputType, 1024, false>);
                                    }
                            }
                            else
                            {
        ...
        void launch_rotate_int8_convrot_weight_kernel(const void* input, void* output,
                                                      int64_t num_rows, int64_t num_cols,
                                                      ...
                                                      int output_dtype_code, hipStream_t stream)
        {
                ...
                {
                        throw std::runtime_error(
                            "convrot rotate kernel only supports group_size 256");
                }
                if (num_cols % comfy::kConvRotGroup != 0)
                {
                        throw std::runtime_error(
                            "convrot rotate kernel requires K divisible by 256");
                }
                if (num_cols > static_cast<int64_t>(std::numeric_limits<int>::max()))
                {
                        throw std::runtime_error(
                            "convrot rotate kernel only supports K <= INT_MAX");
                }

                constexpr int groups_per_block = 8;
                constexpr int block_threads = groups_per_block * 64;
                const int group_blocks = static_cast<int>(
                    (num_cols / comfy::kConvRotGroup + groups_per_block - 1) / groups_per_block);
                const size_t smem_bytes =
                    groups_per_block * 2 * comfy::kConvRotGroup * sizeof(float);
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    [&]
                    {
                            DISPATCH_FP_DTYPE(
                                ...
                                [&]
                                {
                                        comfy::rotate_int8_convrot_groups64_kernel<
                                            groups_per_block, InputType, OutputType>
                ...
                {
                        throw std::runtime_error(
                            std::string("CUDA INT8 convrot rotation failed: ") +
                            hipGetErrorString(err));
                }
        }

        void launch_quantize_int8_convrot_staged_kernel(
            const void* input, void* rotated, void* partial_absmax, void* output, void* scales,
            int64_t num_rows, int64_t num_cols, int group_size, int input_dtype_code,
            int rotated_dtype_code, bool stochastic, uint64_t seed, hipStream_t stream)
        {
                if (num_rows == 0 || num_cols == 0)
                {
                        return;
                ...
                {
                        throw std::runtime_error(
                            "convrot staged quantize only supports group_size 256");
                }
                if (num_cols % comfy::kConvRotGroup != 0)
                {
                        throw std::runtime_error(
                            "convrot staged quantize requires K divisible by 256");
                }
                if (num_cols > static_cast<int64_t>(std::numeric_limits<int>::max()))
                {
                        throw std::runtime_error(
                            "convrot staged quantize only supports K <= INT_MAX");
                }
                ...
                const size_t smem_bytes =
                    groups_per_block * 2 * comfy::kConvRotGroup * sizeof(float);
                const dim3 rotate_grid(static_cast<unsigned int>(num_rows),
                                       static_cast<unsigned int>(group_blocks));
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    {
                            DISPATCH_FP_DTYPE(
                                rotated_dtype_code, RotatedType,
                                [&]
                                {
                                        comfy::rotate_int8_convrot_groups64_amax_kernel<
                                            groups_per_block, InputType, RotatedType>
                ...
                {
                        throw std::runtime_error(
                            std::string("CUDA INT8 staged convrot rotation failed: ") +
                            hipGetErrorString(err));
                }
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    [&]
                    {
                            ...
                            {
                                    kernel<<<static_cast<unsigned int>(num_rows), block_threads, 0,
                                             stream>>>(static_cast<const RotatedType*>(rotated),
                            ...
                            if (quant_threads == 512)
                            {
                                    ...
                                    {
                                            launch(
                                                comfy::quantize_int8_rowwise_from_partials_kernel<
                                                    RotatedType, 512, true>,
                                                512);
                                    }
                                    else
                                    {
                            ...
                            else
                            {
                                    if (stochastic)
                                    {
                                            launch(
                                                comfy::quantize_int8_rowwise_from_partials_kernel<
                                                    RotatedType, comfy::kInt8Threads, true>,
                                                comfy::kInt8Threads);
                                    }
                                    else
                                    {
                                            launch(
                                                comfy::quantize_int8_rowwise_from_partials_kernel<
                                                    RotatedType, comfy::kInt8Threads, false>,
                                                comfy::kInt8Threads);
                                    }
                            }
                    });
                ...
                {
                        throw std::runtime_error(
                            std::string("CUDA INT8 staged convrot quantization failed: ") +
                            hipGetErrorString(err));
                }
        }

        void launch_quantize_int8_rowwise_convrot64_kernel(const void* input, void* output,
                                                           void* scales, int64_t num_rows,
                                                           int64_t num_cols, int group_size,
                                                           int input_dtype_code, bool stochastic,
                                                           ...
                                                           hipStream_t stream)
        {
                ...
                {
                        throw std::runtime_error(
                            "convrot64 fused kernel only supports group_size 256");
                }
                if (num_cols % comfy::kConvRotGroup != 0)
                {
                        throw std::runtime_error(
                            "convrot64 fused kernel requires K divisible by 256");
                }
                if (num_cols > static_cast<int64_t>(std::numeric_limits<int>::max()))
                {
                        throw std::runtime_error(
                            "convrot64 fused kernel only supports K <= INT_MAX");
                }
                if (act_code != comfy::kActNone && act_code != comfy::kActGeluTanh &&
                    act_code != comfy::kActSwiGLU)
                {
                        throw std::runtime_error(
                            "convrot64 fused kernel: unsupported input activation code");
                }
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    [&]
                    {
                            auto launch = [&](auto kernel, int block_threads)
                            {
                                    ...
                                    {
                                            throw std::runtime_error(
                                                std::string("convrot64 fused kernel shared memory "
                                                            "request (") +
                                                std::to_string(smem_bytes) +
                                                " bytes) failed: " + hipGetErrorString(attr_err));
                                    }
                            ...
                            // Same block-size heuristic as before; STOCHASTIC and ACT are turned
                            // into compile-time constants so neither costs a branch in the inner
                            // loop.
                                ...
                                [&]
                                {
                                        ...
                                        switch (act_code)
                                        {
                                                case comfy::kActGeluTanh:
                                                        launch_act(std::integral_constant<
                                                                       int, comfy::kActGeluTanh>{},
                                                                   block_threads);
                                                        break;
                                                case comfy::kActSwiGLU:
                                                        launch_act(std::integral_constant<
                                                                       int, comfy::kActSwiGLU>{},
                                                                   block_threads);
                                                        break;
                                                default:
                                                        launch_act(std::integral_constant<
                ...
                {
                        throw std::runtime_error(
                            std::string("CUDA INT8 rowwise convrot64 quantization failed: ") +
                            hipGetErrorString(err));
                }
        }
        ...
        void launch_dequantize_int8_linear_kernel(const void* input, const void* x_scales,
                                                  ...
                                                  void* output, int64_t num_rows, int64_t num_cols,
                                                  int64_t weight_scale_size, bool has_bias,
                    ...
                    [&]
                    {
                            if (!has_bias)
                            {
                                    if ((num_cols & 3) == 0)
                                    {
                                            ...
                                            const int vec4_blocks = static_cast<int>(
                                                (total_vec4 + kVec4Threads - 1) / kVec4Threads);
                                            comfy::dequantize_int8_linear_vec4_kernel<OutputType>
                                                <<<vec4_blocks, kVec4Threads, 0, stream>>>(
                                                    ...
                                                    static_cast<OutputType*>(output), total_vec4,
                                                    static_cast<int>(num_cols),
        ...
        void launch_int8_gemv_dequant_kernel(const void* input, const void* weight,
                                             const void* x_scales, const void* weight_scales,
                                             const void* bias, void* output, int64_t num_cols,
                                             int64_t K, int64_t weight_scale_size, bool has_bias,
                                             int output_dtype_code, int bias_dtype_code,
                                             hipStream_t stream)
        {
                if (num_cols == 0 || K == 0)
                ...
                {
                        throw std::runtime_error(
                            "INT8 GEMV weight scale must be scalar or per-output-channel");
                }
                    ...
                    [&]
                    {
                            if (!has_bias)
                            {
                                    if ((K & 3) == 0)
                                    {
                                            ...
                                            const unsigned int blocks = static_cast<unsigned int>(
                                                (num_cols + kWarpsPerBlock - 1) / kWarpsPerBlock);
                                            comfy::int8_gemv_dequant_warp_kernel<kWarpsPerBlock,
                                                                                 OutputType, float>
                                                <<<blocks, kWarpsPerBlock * comfy::kThreadsPerWarp,
                                                   0, stream>>>(
                                    ...
                                    {
                                            comfy::int8_gemv_dequant_kernel<comfy::kInt8Threads,
                                                                            OutputType, float>
                                                <<<static_cast<unsigned int>(num_cols),
                                                   comfy::kInt8Threads, 0, stream>>>(
                                                    static_cast<const int8_t*>(input),
                                                    static_cast<const int8_t*>(weight),
                                                    static_cast<const float*>(x_scales),
                                                    static_cast<const float*>(weight_scales),
                            ...
                            DISPATCH_FP_DTYPE(
                                ...
                                [&]
                                {
                                        if ((K & 3) == 0)
                                        {
                                                ...
                                                const unsigned int blocks =
                                                    static_cast<unsigned int>(
                                                        (num_cols + kWarpsPerBlock - 1) /
                                                        kWarpsPerBlock);
                                                comfy::int8_gemv_dequant_warp_kernel<
                                                    kWarpsPerBlock, OutputType, BiasType>
                                                    <<<blocks,
                                                       kWarpsPerBlock * comfy::kThreadsPerWarp, 0,
                                                       stream>>>(
                                                        static_cast<const int8_t*>(input),
                                                        static_cast<const int8_t*>(weight),
                                                        static_cast<const float*>(x_scales),
                                        ...
                                        {
                                                comfy::int8_gemv_dequant_kernel<
                                                    comfy::kInt8Threads, OutputType, BiasType>
                                                    <<<static_cast<unsigned int>(num_cols),
                                                       comfy::kInt8Threads, 0, stream>>>(
                                                        static_cast<const int8_t*>(input),
                                                        static_cast<const int8_t*>(weight),
                ...
                {
                        throw std::runtime_error(
                            std::string("CUDA INT8 GEMV dequantization failed: ") +
                            hipGetErrorString(err));
                }
        }

        void launch_dequantize_int8_simple_kernel(const void* input, const void* scales,
                                                  void* output, int64_t total, int64_t inner_dim,
                                                  int scale_mode, int output_dtype_code,
                                                  hipStream_t stream)
        {
                if (total == 0)
                {
                ...
                if (inner_dim <= 0 ||
                    inner_dim > static_cast<int64_t>(std::numeric_limits<int>::max()))
                {
                        throw std::runtime_error(
                            "dequantize_int8_simple inner dimension is invalid");
                }
                ...
                if (scale_mode == 2 && (total % 4) == 0 && (inner_dim % 4) == 0)
                {
                            ...
                            rows <= static_cast<int64_t>(std::numeric_limits<int>::max()))
                        {
                                const int block_threads =
                                    inner_dim >= 4096 ? 512 : comfy::kInt8Threads;
                                const int blocks_x = static_cast<int>(
                                    (inner_dim_vec4 + block_threads - 1) / block_threads);
                                dim3 grid(static_cast<unsigned int>(rows),
                                          static_cast<unsigned int>(blocks_x));
                                DISPATCH_FP_DTYPE(
                                    output_dtype_code, OutputType,
                                    [&]
                                    {
                                            comfy::dequantize_int8_rowwise_vec4_2d_kernel<
                                                OutputType><<<grid, block_threads, 0, stream>>>(
                                                static_cast<const int8_t*>(input),
                                                static_cast<const float*>(scales),
                                                static_cast<OutputType*>(output),
                                                static_cast<int>(rows), inner_dim_vec4);
                                    });
                                ...
                                if (err != hipSuccess)
                                {
                                        throw std::runtime_error(
                                            std::string(
                                                "CUDA INT8 simple dequantization failed: ") +
                                            hipGetErrorString(err));
                                }
                                return;
                        }

                        const int blocks = static_cast<int>((total_vec4 + comfy::kInt8Threads - 1) /
                                                            comfy::kInt8Threads);
                        DISPATCH_FP_DTYPE(
                            output_dtype_code, OutputType,
                            [&]
                            {
                                    comfy::dequantize_int8_simple_vec4_kernel<OutputType>
                                        <<<blocks, comfy::kInt8Threads, 0, stream>>>(
                                            ...
                                            static_cast<const float*>(scales),
                                            static_cast<OutputType*>(output), total_vec4,
                                            static_cast<int>(inner_dim / 4), scale_mode);
                            });
                        ...
                        {
                                throw std::runtime_error(
                                    std::string("CUDA INT8 simple dequantization failed: ") +
                                    hipGetErrorString(err));
                        }
                        return;
                }
                ...
                if (scale_mode == 0 && (total % 4) == 0)
                {
                        const int64_t total_vec4 = total / 4;
                        const int block_threads =
                            (total_vec4 >= 8'000'000 && total_vec4 <= 16'000'000)
                                ? 512
                                : comfy::kInt8Threads;
                        const int blocks =
                            static_cast<int>((total_vec4 + block_threads - 1) / block_threads);
                        DISPATCH_FP_DTYPE(
                            output_dtype_code, OutputType,
                            [&]
                            {
                                    comfy::dequantize_int8_simple_vec4_kernel<OutputType>
                                        <<<blocks, block_threads, 0, stream>>>(
                                            static_cast<const int8_t*>(input),
                                            static_cast<const float*>(scales),
                                            static_cast<OutputType*>(output), total_vec4, 1,
                                            scale_mode);
                            });
                                  ...
                                  [&]
                                  {
                                          comfy::dequantize_int8_simple_kernel<OutputType>
                                              <<<blocks, comfy::kInt8Threads, 0, stream>>>(
                                                  ...
                                                  static_cast<const float*>(scales),
                                                  static_cast<OutputType*>(output), total,
                                                  static_cast<int>(inner_dim), scale_mode);
                                  });
        ...
        void launch_dequantize_int8_convrot_kernel(const void* input, const void* scales,
                                                   void* output, int64_t num_rows, int64_t num_cols,
                                                   int64_t scale_size, int group_size,
                                                   int output_dtype_code, hipStream_t stream)
        {
                if (num_rows == 0 || num_cols == 0)
                {
                ...
                {
                        throw std::runtime_error(
                            "convrot dequant kernel only supports group_size 256");
                }
                if (num_cols % comfy::kConvRotGroup != 0)
                {
                        throw std::runtime_error(
                            "convrot dequant kernel requires K divisible by 256");
                }
                if (num_cols > static_cast<int64_t>(std::numeric_limits<int>::max()))
                {
                        throw std::runtime_error(
                            "convrot dequant kernel only supports K <= INT_MAX");
                }
                ...
                if (num_cols >= comfy::kConvRotGroup)
                {
                        auto launch_groups = [&](auto groups_tag)
                        {
                                ...
                                const int group_blocks = static_cast<int>(
                                    (num_cols / comfy::kConvRotGroup + groups_per_block - 1) /
                                    groups_per_block);
                                const size_t smem_bytes =
                                    groups_per_block * 2 * comfy::kConvRotGroup * sizeof(float);
                                const dim3 grid(static_cast<unsigned int>(num_rows),
                                ...
                                DISPATCH_FP_DTYPE(
                                    ...
                                    [&]
                                    {
                                            // Re-derived from the tag: MSVC 14.42 refuses to
                                            // capture the enclosing lambda's constexpr local
                                            // (C3495).
                                            constexpr int kGroupsPerBlock =
                                                decltype(groups_tag)::value;
                                            comfy::dequantize_int8_convrot_groups64_kernel<
                                                kGroupsPerBlock, OutputType>
                                                <<<grid, block_threads, smem_bytes, stream>>>(
                                                    static_cast<const int8_t*>(input),
                                                    static_cast<const float*>(scales),
                                                    static_cast<OutputType*>(output),
                        ...
                        {
                                throw std::runtime_error(
                                    std::string("CUDA INT8 convrot dequantization failed: ") +
                                    hipGetErrorString(err));
                        }
                        return;
                }
                ...
                auto launch = [&](auto kernel, auto* output_typed)
                {
                        ...
                        kernel<<<static_cast<unsigned int>(num_rows), block_threads, smem_bytes,
                                 stream>>>(
                            static_cast<const int8_t*>(input), static_cast<const float*>(scales),
                            output_typed, static_cast<int>(num_cols), static_cast<int>(scale_size));
                };
                DISPATCH_FP_DTYPE(
                    output_dtype_code, OutputType,
                    [&]
                    {
                            if (wide)
                            {
                                    launch(comfy::dequantize_int8_convrot_kernel<1024, OutputType>,
                                           static_cast<OutputType*>(output));
                            }
                            else
                            {
                                    launch(comfy::dequantize_int8_convrot_kernel<256, OutputType>,
                                           static_cast<OutputType*>(output));
                            }
                    });
