...
template <> __device__ __forceinline__ float to_float<__hip_bfloat16>(__hip_bfloat16 val) {
    return __bfloat162float(val);
}

template <typename T> __device__ __forceinline__ float finite_max_for_dtype();

template <> __device__ __forceinline__ float finite_max_for_dtype<float>() {
    return FLT_MAX;
}

template <> __device__ __forceinline__ float finite_max_for_dtype<half>() {
    return 65504.0f;
}

template <> __device__ __forceinline__ float finite_max_for_dtype<__hip_bfloat16>() {
    return 3.38953139e38f;
}

template <typename T> __device__ __forceinline__ float finite_absmax_for_int8_scale(float abs_max) {
...
template <> __device__ __forceinline__ __hip_bfloat16 from_float<__hip_bfloat16>(float val) {
    return __float2bfloat16(val);
}

template <typename T> __device__ __forceinline__ float quant_div_to_float(T val, float scale) {
    const float scale_t = to_float(from_float<T>(scale));
    return to_float(from_float<T>(to_float(val) / scale_t));
}

template <typename T> __device__ __forceinline__ float quant_div_float_to_float(float val, float scale) {
    const float scale_t = to_float(from_float<T>(scale));
    return to_float(from_float<T>(to_float(from_float<T>(val)) / scale_t));
}

template <typename T> __device__ __forceinline__ float stochastic_sum_to_float(float scaled, T rng) {
    return to_float(from_float<T>(scaled + to_float(rng)));
}

template <> __device__ __forceinline__ float stochastic_sum_to_float<float>(float scaled, float rng) {
    return scaled + rng;
}

template <> __device__ __forceinline__ float stochastic_sum_to_float<half>(float scaled, half rng) {
    return __half2float(__hadd(__float2half_rn(scaled), rng));
}

template <> __device__ __forceinline__ float stochastic_sum_to_float<__hip_bfloat16>(float scaled, __hip_bfloat16 rng) {
...
template <typename T> __device__ __forceinline__ T stochastic_rng_value(int64_t idx, uint64_t seed) {
...
template <>
__device__ __forceinline__ void store4_contiguous<__hip_bfloat16>(__hip_bfloat16 * out,
                                                                  int64_t          idx,
                                                                  float            x,
                                                                  float            y,
                                                                  float            z,
...
__global__ void quantize_int8_rowwise_kernel(const InputType * __restrict__ x,
    ...
    const float scale = fmaxf(finite_absmax_for_int8_scale<InputType>(abs_max) * (1.0f / 127.0f), 1.0e-30f);
...
#pragma unroll
        for (int stage = 0; stage < 4; ++stage) {
            ...
            const float v    = 0.5f * h4_row_dot(d, src[base], src[base + s], src[base + 2 * s], src[base + 3 * s]);
            dst[i]           = v;
            __syncthreads();
            float * t = src;
            src       = dst;
...
__global__ void dequantize_int8_linear_vec4_kernel(const int32_t * __restrict__ input,
    ...
    if (weight_scale_size == 1) {
        ...
        store4_contiguous(output, idx, static_cast<float>(acc.x) * scale, static_cast<float>(acc.y) * scale,
                          static_cast<float>(acc.z) * scale, static_cast<float>(acc.w) * scale);
        return;
    }
    ...
    store4_contiguous(output, idx, static_cast<float>(acc.x) * x_scale * ws.x,
                      static_cast<float>(acc.y) * x_scale * ws.y, static_cast<float>(acc.z) * x_scale * ws.z,
...
__global__ void dequantize_int8_simple_vec4_kernel(const int8_t * __restrict__ input,
    ...
    const int64_t idx   = idx4 * 4;
    store4_contiguous(output, idx, static_cast<float>(q4.x) * scale, static_cast<float>(q4.y) * scale,
                      static_cast<float>(q4.z) * scale, static_cast<float>(q4.w) * scale);
}
...
__global__ void rotate_int8_convrot_groups64_amax_kernel(const InputType * __restrict__ x,
    ...
    if (active) {
        local_max = convrot_fht_stage64_store_absmax<64, OutputType>(buf1, output + row_offset + group_col, lane);
    }
    buf0[lane] = local_max;
    __syncthreads();
...
template <int ACT> __device__ __forceinline__ float apply_input_act(float v) {
...
__device__ __forceinline__ float load_input_act(const InputType * __restrict__ x, int64_t in_row, int col, int K) {
...
__global__ void quantize_int8_rowwise_convrot64_kernel(const InputType * __restrict__ x,
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        if (active) {
            abs_max = fmaxf(abs_max, convrot_fht_stage64_store_absmax<64, float>(buf1, row_buf + group_col, lane));
        }
        __syncthreads();
    }
...
extern "C" {
void launch_quantize_int8_rowwise_kernel(const void * input,
                                         void *       output,
                                         void *       scales,
                                         int64_t      num_rows,
                                         int64_t      num_cols,
    ...
    DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
        auto launch = [&](auto kernel, int block_threads) {
            kernel<<<static_cast<unsigned int>(num_rows), block_threads, 0, stream>>>(
                static_cast<const InputType *>(input), static_cast<int8_t *>(output), static_cast<float *>(scales),
        ...
        if (num_cols >= 4096 && num_rows != 1) {
            ...
            if (stochastic) {
                launch(comfy::quantize_int8_rowwise_kernel<InputType, comfy::kInt8Threads, true>, comfy::kInt8Threads);
            } else {
                launch(comfy::quantize_int8_rowwise_kernel<InputType, comfy::kInt8Threads, false>, comfy::kInt8Threads);
            }
        }
    });
    ...
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 rowwise quantization failed: ") + hipGetErrorString(err));
    }
}

void launch_quantize_int8_rowwise_convrot_kernel(const void * input,
                                                 void *       output,
                                                 void *       scales,
                                                 int64_t      num_rows,
                                                 int64_t      num_cols,
    ...
    DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
        auto launch = [&](auto kernel) {
            ...
            if (attr_err != hipSuccess) {
                throw std::runtime_error(std::string("convrot fused kernel shared memory "
                                                     "request (") +
                                         std::to_string(smem_bytes) + " bytes) failed: " + hipGetErrorString(attr_err));
            }
    ...
    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 rowwise convrot quantization failed: ") +
                                 hipGetErrorString(err));
    }
}

void launch_rotate_int8_convrot_weight_kernel(const void * input,
                                              void *       output,
                                              int64_t      num_rows,
                                              int64_t      num_cols,
                                              int          group_size,
    ...
    const dim3   grid(static_cast<unsigned int>(num_rows), static_cast<unsigned int>(group_blocks));
    ...
    DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
        DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
            comfy::rotate_int8_convrot_groups64_kernel<groups_per_block, InputType, OutputType>
                <<<grid, block_threads, smem_bytes, stream>>>(static_cast<const InputType *>(input),
    ...
    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 convrot rotation failed: ") + hipGetErrorString(err));
    }
}

void launch_quantize_int8_convrot_staged_kernel(const void * input,
                                                void *       rotated,
                                                void *       partial_absmax,
                                                void *       output,
                                                void *       scales,
    ...
    const int     group_blocks =
        static_cast<int>((num_cols / comfy::kConvRotGroup + groups_per_block - 1) / groups_per_block);
    const size_t smem_bytes = groups_per_block * 2 * comfy::kConvRotGroup * sizeof(float);
    const dim3   rotate_grid(static_cast<unsigned int>(num_rows), static_cast<unsigned int>(group_blocks));
        ...
        DISPATCH_FP_DTYPE(rotated_dtype_code, RotatedType, [&] {
            comfy::rotate_int8_convrot_groups64_amax_kernel<groups_per_block, InputType, RotatedType>
                <<<rotate_grid, rotate_threads, smem_bytes, stream>>>(
                    static_cast<const InputType *>(input), static_cast<RotatedType *>(rotated),
                    static_cast<float *>(partial_absmax), static_cast<int>(num_cols));
        });
    });

    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 staged convrot rotation failed: ") + hipGetErrorString(err));
    }
    ...
    DISPATCH_FP_DTYPE(rotated_dtype_code, RotatedType, [&] {
        auto launch = [&](auto kernel, int block_threads) {
            kernel<<<static_cast<unsigned int>(num_rows), block_threads, 0, stream>>>(
                static_cast<const RotatedType *>(rotated), static_cast<const float *>(partial_absmax),
                static_cast<int8_t *>(output), static_cast<float *>(scales), static_cast<int>(num_cols), seed);
        };
    ...
    err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 staged convrot quantization failed: ") +
                                 hipGetErrorString(err));
    }
}

void launch_quantize_int8_rowwise_convrot64_kernel(const void * input,
                                                   void *       output,
                                                   void *       scales,
                                                   int64_t      num_rows,
                                                   int64_t      num_cols,
        ...
        auto launch = [&](auto kernel, int block_threads) {
            ...
            hipError_t attr_err =
                hipFuncSetAttribute(reinterpret_cast<const void *>(kernel), hipFuncAttributeMaxDynamicSharedMemorySize,
                                    static_cast<int>(smem_bytes));
            if (attr_err != hipSuccess) {
                throw std::runtime_error(std::string("convrot64 fused kernel shared memory "
                                                     "request (") +
                                         std::to_string(smem_bytes) + " bytes) failed: " + hipGetErrorString(attr_err));
            }
            kernel<<<static_cast<unsigned int>(num_rows), block_threads, smem_bytes, stream>>>(
    ...
    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 rowwise convrot64 quantization failed: ") +
                                 hipGetErrorString(err));
    }
}

void launch_dequantize_int8_linear_kernel(const void * input,
                                          const void * x_scales,
    ...
    DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
        if (!has_bias) {
            if ((num_cols & 3) == 0) {
                ...
                comfy::dequantize_int8_linear_vec4_kernel<OutputType><<<vec4_blocks, kVec4Threads, 0, stream>>>(
                    static_cast<const int32_t *>(input), static_cast<const float *>(x_scales),
                    static_cast<const float *>(weight_scales), static_cast<OutputType *>(output), total_vec4,
                    static_cast<int>(num_cols), static_cast<int>(weight_scale_size));
            } else {
                comfy::dequantize_int8_linear_kernel<OutputType, float><<<blocks, comfy::kInt8Threads, 0, stream>>>(
                    static_cast<const int32_t *>(input), static_cast<const float *>(x_scales),
                    static_cast<const float *>(weight_scales), nullptr, static_cast<OutputType *>(output), total,
                    static_cast<int>(num_cols), static_cast<int>(weight_scale_size), false);
            }
            return;
        }

        DISPATCH_FP_DTYPE(bias_dtype_code, BiasType, [&] {
            comfy::dequantize_int8_linear_kernel<OutputType, BiasType><<<blocks, comfy::kInt8Threads, 0, stream>>>(
                static_cast<const int32_t *>(input), static_cast<const float *>(x_scales),
                static_cast<const float *>(weight_scales), static_cast<const BiasType *>(bias),
                static_cast<OutputType *>(output), total, static_cast<int>(num_cols),
    ...
    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 linear dequantization failed: ") + hipGetErrorString(err));
    }
}

void launch_int8_gemv_dequant_kernel(const void * input,
                                     const void * weight,
                                     const void * x_scales,
                                     const void * weight_scales,
                                     const void * bias,
    ...
    DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
        if (!has_bias) {
            if ((K & 3) == 0) {
                ...
                const unsigned int blocks = static_cast<unsigned int>((num_cols + kWarpsPerBlock - 1) / kWarpsPerBlock);
                comfy::int8_gemv_dequant_warp_kernel<kWarpsPerBlock, OutputType, float>
                    <<<blocks, kWarpsPerBlock * comfy::kThreadsPerWarp, 0, stream>>>(
                        ...
                        static_cast<const float *>(x_scales), static_cast<const float *>(weight_scales), nullptr,
                        static_cast<OutputType *>(output), static_cast<int>(num_cols), static_cast<int>(K),
            ...
            if ((K & 3) == 0) {
                ...
                comfy::int8_gemv_dequant_warp_kernel<kWarpsPerBlock, OutputType, BiasType>
                    <<<blocks, kWarpsPerBlock * comfy::kThreadsPerWarp, 0, stream>>>(
                        ...
                        static_cast<const BiasType *>(bias), static_cast<OutputType *>(output),
                        static_cast<int>(num_cols), static_cast<int>(K), static_cast<int>(weight_scale_size), true);
            } else {
    ...
    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
        throw std::runtime_error(std::string("CUDA INT8 GEMV dequantization failed: ") + hipGetErrorString(err));
    }
}

void launch_dequantize_int8_simple_kernel(const void * input,
                                          const void * scales,
                                          void *       output,
                                          int64_t      total,
                                          int64_t      inner_dim,
    ...
    if (scale_mode == 2 && (total % 4) == 0 && (inner_dim % 4) == 0) {
        ...
        if (inner_dim >= 1024 && rows <= static_cast<int64_t>(std::numeric_limits<int>::max())) {
            ...
            DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
                comfy::dequantize_int8_rowwise_vec4_2d_kernel<OutputType><<<grid, block_threads, 0, stream>>>(
                    static_cast<const int8_t *>(input), static_cast<const float *>(scales),
                    static_cast<OutputType *>(output), static_cast<int>(rows), inner_dim_vec4);
            });

            hipError_t err = hipGetLastError();
            if (err != hipSuccess) {
                throw std::runtime_error(std::string("CUDA INT8 simple dequantization failed: ") +
                                         hipGetErrorString(err));
            }
            return;
        }
        ...
        DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
            comfy::dequantize_int8_simple_vec4_kernel<OutputType><<<blocks, comfy::kInt8Threads, 0, stream>>>(
                static_cast<const int8_t *>(input), static_cast<const float *>(scales),
                static_cast<OutputType *>(output), total_vec4, static_cast<int>(inner_dim / 4), scale_mode);
        });

        hipError_t err = hipGetLastError();
        if (err != hipSuccess) {
            throw std::runtime_error(std::string("CUDA INT8 simple dequantization failed: ") + hipGetErrorString(err));
        }
        return;
    }
    ...
    if (scale_mode == 0 && (total % 4) == 0) {
        const int64_t total_vec4    = total / 4;
        const int     block_threads = (total_vec4 >= 8'000'000 && total_vec4 <= 16'000'000) ? 512 : comfy::kInt8Threads;
        const int     blocks        = static_cast<int>((total_vec4 + block_threads - 1) / block_threads);
        DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
            comfy::dequantize_int8_simple_vec4_kernel<OutputType><<<blocks, block_threads, 0, stream>>>(
                static_cast<const int8_t *>(input), static_cast<const float *>(scales),
                static_cast<OutputType *>(output), total_vec4, 1, scale_mode);
        });
    ...
    const int blocks = static_cast<int>((total + comfy::kInt8Threads - 1) / comfy::kInt8Threads);
    DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
        comfy::dequantize_int8_simple_kernel<OutputType><<<blocks, comfy::kInt8Threads, 0, stream>>>(
            static_cast<const int8_t *>(input), static_cast<const float *>(scales), static_cast<OutputType *>(output),
            total, static_cast<int>(inner_dim), scale_mode);
    });
...
void launch_dequantize_int8_convrot_kernel(const void * input,
                                           const void * scales,
                                           void *       output,
                                           int64_t      num_rows,
                                           int64_t      num_cols,
    ...
    if (num_cols >= comfy::kConvRotGroup) {
            ...
            DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
                ...
                comfy::dequantize_int8_convrot_groups64_kernel<kGroupsPerBlock, OutputType>
                    <<<grid, block_threads, smem_bytes, stream>>>(
                        static_cast<const int8_t *>(input), static_cast<const float *>(scales),
                        static_cast<OutputType *>(output), static_cast<int>(num_cols), static_cast<int>(scale_size));
            });
        };
        ...
        hipError_t err = hipGetLastError();
        if (err != hipSuccess) {
            throw std::runtime_error(std::string("CUDA INT8 convrot dequantization failed: ") + hipGetErrorString(err));
        }
        return;
    }
    ...
    auto launch = [&](auto kernel, auto * output_typed) {
        hipError_t attr_err =
            hipFuncSetAttribute(reinterpret_cast<const void *>(kernel), hipFuncAttributeMaxDynamicSharedMemorySize,
                                static_cast<int>(smem_bytes));
        if (attr_err != hipSuccess) {
            throw std::runtime_error(std::string("convrot dequant kernel shared memory request (") +
                                     std::to_string(smem_bytes) + " bytes) failed: " + hipGetErrorString(attr_err));
        }
        kernel<<<static_cast<unsigned int>(num_rows), block_threads, smem_bytes, stream>>>(
            static_cast<const int8_t *>(input), static_cast<const float *>(scales), output_typed,
            static_cast<int>(num_cols), static_cast<int>(scale_size));
    };
    DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
        if (wide) {
