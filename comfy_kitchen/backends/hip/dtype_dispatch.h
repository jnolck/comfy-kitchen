...
template <int DTypeCode>
struct DTypeMap;

template <>
struct DTypeMap<DTYPE_CODE_FLOAT32>
{
        using type = float;
};
template <>
struct DTypeMap<DTYPE_CODE_FLOAT16>
{
        using type = half;
};
template <>
struct DTypeMap<DTYPE_CODE_BFLOAT16>
{
...
template <>
struct FP8DTypeMap<DTYPE_CODE_FLOAT8_E4M3FN>
{
        using type = __hip_fp8_e4m3_fnuz;
};
template <>
struct FP8DTypeMap<DTYPE_CODE_FLOAT8_E5M2>
{
        using type = __hip_fp8_e5m2_fnuz;
};
...
inline hipDataType dtype_code_to_cuda_type(int dtype_code)
{
        switch (dtype_code)
        {
                case DTYPE_CODE_FLOAT32:
                        return HIP_R_32F;
                case DTYPE_CODE_FLOAT16:
                        return HIP_R_16F;
                case DTYPE_CODE_BFLOAT16:
                        return HIP_R_16BF;
                default:
                        throw std::runtime_error("Unsupported dtype code for CUDA type: " +
...
#define DISPATCH_INPUT_DTYPE(dtype_code, type_name, ...)                                    \
        ...
        {                                                                                   \
                ...
                {                                                                           \
                        ...
                        {                                                                   \
                                using type_name = __hip_bfloat16;                             \
                                return __VA_ARGS__();                                       \
                        }                                                                   \
                        default:                                                            \
                                throw std::runtime_error("Unsupported input dtype code: " + \
                                                         std::to_string(dtype_code));       \
                }                                                                           \
        }()
...
#define DISPATCH_FP8_OUTPUT_DTYPE(dtype_code, type_name, ...)                                    \
        ...
        {                                                                                        \
                ...
                {                                                                                \
                        ...
                        {                                                                        \
                                using type_name = __hip_fp8_e4m3_fnuz;                           \
                                return __VA_ARGS__();                                            \
                        }                                                                        \
                        case comfy::DTYPE_CODE_FLOAT8_E5M2:                                      \
                        {                                                                        \
                                using type_name = __hip_fp8_e5m2_fnuz;                           \
                                return __VA_ARGS__();                                            \
                        }                                                                        \
                        default:                                                                 \
                                throw std::runtime_error("Unsupported FP8 output dtype code: " + \
                                                         std::to_string(dtype_code));            \
                }                                                                                \
        }()
...
#define DISPATCH_INPUT_FP8_OUTPUT_DTYPES(in_dtype_code, out_dtype_code, in_type_name, \
                                         out_type_name, ...)                          \
        DISPATCH_INPUT_DTYPE(                                                         \
            in_dtype_code, in_type_name,                                              \
            [&] { return DISPATCH_FP8_OUTPUT_DTYPE(out_dtype_code, out_type_name, __VA_ARGS__); })
...
//                                               { /* code using InputType and OutputType */ });
#define DISPATCH_FP8_INPUT_HALF_OUTPUT_DTYPES(in_dtype_code, out_dtype_code, in_type_name, \
                                              out_type_name, ...)                          \
        DISPATCH_FP8_OUTPUT_DTYPE(                                                         \
            in_dtype_code, in_type_name,                                                   \
            [&] { return DISPATCH_HALF_DTYPE(out_dtype_code, out_type_name, __VA_ARGS__); })
...
//                                            { /* code using InputType and OutputType */ });
#define DISPATCH_FP8_INPUT_FP_OUTPUT_DTYPES(in_dtype_code, out_dtype_code, in_type_name, \
                                            out_type_name, ...)                          \
        DISPATCH_FP8_OUTPUT_DTYPE(                                                       \
            in_dtype_code, in_type_name,                                                 \
            [&] { return DISPATCH_FP_DTYPE(out_dtype_code, out_type_name, __VA_ARGS__); })
...
#define DISPATCH_HALF_INPUT_FP_FREQS_DTYPES(in_dtype_code, freqs_dtype_code, in_type_name, \
                                            freqs_type_name, ...)                          \
        DISPATCH_HALF_DTYPE(                                                               \
            in_dtype_code, in_type_name,                                                   \
            [&] { return DISPATCH_FP_DTYPE(freqs_dtype_code, freqs_type_name, __VA_ARGS__); })
