...
template <uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim,
          DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
          typename DTypeSVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half,
          ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone,
          bool return_lse = false, bool fuse_v_scale = false, bool fuse_v_mean = false,
          bool use_pv_fp16_accu = false, bool fuse_fp32_probabilities = true>
__global__ void qk_int_sv_i8_attn_kernel(
    int8_t* __restrict__ Q, int8_t* __restrict__ K, int8_t* __restrict__ V,
    DTypeOut* __restrict__ O, float* __restrict__ Lse, float* __restrict__ Q_scale,
    float* __restrict__ K_scale, float* __restrict__ V_scale, float* __restrict__ V_mean,
    const void* __restrict__ AttnMask, const int64_t mask_stride_b, const int64_t mask_stride_h,
    const int64_t mask_stride_q, const int64_t mask_stride_k, const int mask_dtype_code,
    ...
    float sm_scale)
{
        ...
        static_assert(
            std::is_same<DTypeOut, half>::value || std::is_same<DTypeOut, __hip_bfloat16>::value,
            "DTypeOut must be half or hip_bfloat16");
        ...
        constexpr bool custom_mask =
            mask_mode == MaskMode::kCustom || mask_mode == MaskMode::kCustomKey;
        ...
        else if constexpr (Q_GRAN == QuantGranularity::kPerThread)
        {
                if constexpr (head_dim == 128 && WARP_Q == 16)
                {
                        ...
                        q_scale_idx = batch_id * num_qo_heads * (num_warp_block_q * 8) +
                                      ...
                                      (get_warp_idx_q<num_warps_q, num_warps_k>() / 2) * 8 +
                                      lane_id / 4;
                }
                else
                {
...
#pragma unroll
        ...
        smem_t<swizzle_mode_QK, QK_SMEM_STRIDE / PACK_SIZE_QK> smem_K(smem + K_smem_idx_offset *
                                                                                 QK_SMEM_STRIDE);
        constexpr SwizzleMode swizzle_mode_V =
            (V_SMEM_STRIDE == 64) ? SwizzleMode::k64B : SwizzleMode::k128B;
        smem_t<swizzle_mode_V, V_SMEM_STRIDE / PACK_SIZE_V> smem_V(smem + V_smem_idx_offset *
                                                                              QK_SMEM_STRIDE);
        ...
        int8_t* Q_lane_base_ptr =
            ...
            (bx * CTA_Q + CTA_Q / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK) *
                stride_seq_q +
        ...
        int8_t* K_lane_base_ptr =
            ...
            (CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK) *
                stride_seq_k +
            (lane_id % global_to_shared_line_lanes_QK) * PACK_SIZE_QK;
        int8_t* V_lane_base_ptr = V + batch_id * stride_bz_v +
        ...
        uint32_t Q_idx_lane_base =
            bx * CTA_Q + get_warp_idx_q<num_warps_q, num_warps_k>() * WARP_Q + lane_id / 4;
        ...
        const uint32_t num_iterations = div_ceil(
            mask_mode == MaskMode::kCausal ? min(kv_len, (bx + 1) * CTA_Q) : kv_len, CTA_K);

        // load Q with predicate
        load_global_to_share<global_to_shared_line_lanes_QK,
                             global_to_shared_copy_lines_per_warp_QK, QK_smem_iters_row,
                             Q_smem_iters_col, swizzle_mode_QK, QK_SMEM_STRIDE / PACK_SIZE_QK,
                             CTA_Q>(&Q_lane_base_ptr, Q_smem_offset_load, stride_seq_q, smem_Q,
...
#pragma unroll
        ...
        load_global_to_share<global_to_shared_line_lanes_QK,
                             global_to_shared_copy_lines_per_warp_QK, QK_smem_iters_row,
                             K_smem_iters_col, swizzle_mode_QK, QK_SMEM_STRIDE / PACK_SIZE_QK,
        ...
        load_int8_V_global_to_share<
            global_to_shared_line_lanes_V, global_to_shared_copy_lines_per_warp_V, V_smem_iters_row,
            V_smem_iters_col, swizzle_mode_V, V_SMEM_STRIDE / PACK_SIZE_V, CTA_K>(
...
#pragma unroll
        for (uint32_t iter = 1; iter < num_iterations - 1; iter++)
        {
                ...
                if constexpr (use_fused_fp32_probabilities && mask_mode == MaskMode::kNone)
                {
...
#pragma unroll
                        ...
                        update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, true, pre_scale_scores>(
                            RS_soft, RO, m, d, pv_scale, sm_scale, S_U8_OFFSET);
...
#pragma unroll
                ...
                load_global_to_share<global_to_shared_line_lanes_QK,
                                     global_to_shared_copy_lines_per_warp_QK, QK_smem_iters_row,
                                     K_smem_iters_col, swizzle_mode_QK,
                ...
                compute_int8_sv<num_warps_q, num_warps_k, num_tiles_q, num_tiles_k, num_tiles_v,
                                swizzle_mode_V, V_SMEM_STRIDE / PACK_SIZE_V>(smem_V, RS, RS_u8, RO);
                __syncthreads();
                // load V
...
#pragma unroll
                ...
                {
                        apply_custom_key_mask<num_tiles_q, num_tiles_k>(
                            K_idx_lane_base, RS_soft, valid, AttnMask, mask_stride_b, mask_stride_h,
                            mask_stride_k, batch_id, head_id, kv_len, mask_dtype_code, 1.0f);
                }
                apply_out_of_bound_mask<num_tiles_q, num_tiles_k>(
                    K_idx_lane_base, RS_soft, kv_len, pre_scale_scores ? -50000.0f : -1.0e30f);
...
#pragma unroll
                        for (uint32_t k = 0; k < 2; k++)
                        {
                                ...
                                row_valid |= __shfl_xor_sync(0xfffffffful, row_valid, 0x1);
                                row_valid |= __shfl_xor_sync(0xfffffffful, row_valid, 0x2);
...
#pragma unroll
                                for (uint32_t k = 0; k < 8; k++)
                                {
                                        if (!valid[fq][(k % 4) / 2]) RO[fq][fv][k] = 0.0f;
                                }
                        }
                }
        }
        ...
        {
                float v_scale[4];
                float* V_scale_base_ptr = V_scale +
                                          batch_id * (num_qo_heads / num_kv_groups) * head_dim +
...
#pragma unroll
                                for (uint32_t k = 0; k < 4; k++)
                                {
                                        ...
                                        {
                                                ((half2*)RO_f16)[k] =
                                                    __float22half2_rn(((float2*)RO[fq][fv])[k]);
                                        }
                                        else
                                        {
                                                ((__hip_bfloat162*)RO_f16)[k] =
                                ...
                                offset_O =
                                    smem_O.get_permuted_offset(smem_O_row_base + fq * MMA_QK_M,
                                                               fv * (MMA_SV_N / PACK_SIZE_O) + 1);
        ...
        DTypeOut* O_lane_ptr = O + batch_id * stride_bz_o + head_id * stride_h_o +
        ...
        uint32_t offset_O =
            smem_O.get_permuted_offset(get_warp_idx_q<num_warps_q, num_warps_k>() * WARP_Q +
                                           lane_id / global_to_shared_line_lanes_O,
                                       lane_id % global_to_shared_line_lanes_O);
        uint32_t O_load_idx_lane_base =
            bx * CTA_Q + CTA_Q / num_warps * warp_id + lane_id / global_to_shared_line_lanes_O;
...
#pragma unroll
                for (uint32_t j = 0; j < O_smem_iters_row; j++)
                {
                        ...
                        offset_O =
                            smem_O.template advance_offset_by_column<global_to_shared_line_lanes_O>(
                                offset_O);
                }

                offset_O =
                    smem_O.template advance_offset_by_row<global_to_shared_copy_lines_per_warp_O>(
