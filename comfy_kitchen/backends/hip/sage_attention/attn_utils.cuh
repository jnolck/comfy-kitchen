...
template <uint32_t global_to_shared_line_lanes,
          uint32_t global_to_shared_copy_lines_per_warp_per_iter, uint32_t smem_iters_row,
          uint32_t smem_iters_col, SwizzleMode swizzle_mode, uint32_t stride, uint32_t CTA,
          typename T>
...
{
        static_assert(global_to_shared_copy_lines_per_warp_per_iter * global_to_shared_line_lanes ==
                      WARP_SIZE);
...
#pragma unroll
        ...
        *lane_ptr +=
            (CTA - smem_iters_col * global_to_shared_copy_lines_per_warp_per_iter) * gmem_stride;
}
...
template <uint32_t global_to_shared_line_lanes,
          uint32_t global_to_shared_copy_lines_per_warp_per_iter, uint32_t smem_iters_row,
          uint32_t smem_iters_col, SwizzleMode swizzle_mode, uint32_t stride, uint32_t CTA>
__device__ __forceinline__ void load_int8_V_global_to_share(
...
template <uint32_t num_warps_q, uint32_t num_warps_k, uint32_t num_tiles_q, uint32_t num_tiles_k,
          uint32_t num_tiles_qk_inner, SwizzleMode swizzle_mode, uint32_t stride, DataType DTypeQK>
...
#pragma unroll
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                ...
                                {
                                        mma::mma_sync_m16n16k32_row_col_s8s8s32<
                                            mma::MMAMode::kInit>(RS[fq][fk], RQ[fq], RK);
                                }
                                else if constexpr (DTypeQK == DataType::kInt4)
                                {
                                        mma::mma_sync_m16n16k64_row_col_s4s4s32<
                                            mma::MMAMode::kInit>(RS[fq][fk], RQ[fq], RK);
                                }
                        }
                }
...
#pragma unroll
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                ...
                                {
                                        mma::mma_sync_m16n16k32_row_col_s8s8s32<
                                            mma::MMAMode::kInplaceUpdate>(RS[fq][fk], RQ[fq], RK);
                                }
                                else if constexpr (DTypeQK == DataType::kInt4)
                                {
                                        mma::mma_sync_m16n16k64_row_col_s4s4s32<
...
__device__ __forceinline__ void apply_causal_mask(const uint32_t& Q_idx_lane_base,
...
#pragma unroll
                        for (uint32_t k = 0; k < 8; k++)
                        {
                                const uint32_t q_idx =
                                    Q_idx_lane_base + fq * 16 + 8 * ((k % 4) / 2);
                                const uint32_t kv_idx =
                                    K_idx_lane_base + fk * 16 + 8 * (k / 4) + k % 2;
...
__device__ __forceinline__ void apply_custom_mask(
    const uint32_t& Q_idx_lane_base, const uint32_t& K_idx_lane_base, float RS[][num_tiles_k][8],
    bool valid[][2], const void* mask, const int64_t mask_stride_b, const int64_t mask_stride_h,
    const int64_t mask_stride_q, const int64_t mask_stride_k, const uint32_t batch_id,
    const uint32_t head_id, const uint32_t qo_len, const uint32_t kv_len, const int mask_dtype_code,
...
#pragma unroll
                        for (uint32_t k = 0; k < 8; k++)
                        {
                                ...
                                if (keep)
                                {
                                        ...
                                        {
                                                keep =
                                                    static_cast<const uint8_t*>(mask)[offset] != 0;
                                        }
                                        else
                                        {
                                                ...
                                                {
                                                        bias =
                                                            static_cast<const float*>(mask)[offset];
                                                }
                                                else if (mask_dtype_code == 1)
                                                {
                                                        bias = static_cast<float>(
                                                            static_cast<const half*>(mask)[offset]);
                                                }
                                                else
                                                {
                                                        bias = static_cast<float>(
                                ...
                                RS[fq][fk][k] =
                                    keep ? fmaf(RS[fq][fk][k], score_scale, bias * math::log2e)
                                         : -50000.0f;
                        }
                }
        }
...
__device__ __forceinline__ void apply_custom_key_mask(
    const uint32_t& K_idx_lane_base, float RS[][num_tiles_k][8], bool valid[][2], const void* mask,
    const int64_t mask_stride_b, const int64_t mask_stride_h, const int64_t mask_stride_k,
    const uint32_t batch_id, const uint32_t head_id, const uint32_t kv_len,
    const int mask_dtype_code, const float score_scale)
{
        const int64_t head_offset = static_cast<int64_t>(batch_id) * mask_stride_b +
                                    static_cast<int64_t>(head_id) * mask_stride_h;
...
#pragma unroll
                ...
                {
                        const uint32_t kv_idx =
                            K_idx_lane_base + fk * 16 + (value >> 1) * 8 + (value & 1);
                        keep[value] = kv_idx < kv_len;
                        bias[value] = 0.0f;
                        if (keep[value])
                        {
                                const int64_t offset =
                                    head_offset + static_cast<int64_t>(kv_idx) * mask_stride_k;
                                if (mask_dtype_code == 3)
                                {
                                        keep[value] =
                                            static_cast<const uint8_t*>(mask)[offset] != 0;
...
#pragma unroll
                        for (uint32_t k = 0; k < 8; k++)
                        {
                                ...
                                RS[fq][fk][k] = keep[value] ? fmaf(RS[fq][fk][k], score_scale,
...
template <uint32_t num_tiles_q, uint32_t num_tiles_k>
__device__ __forceinline__ void apply_out_of_bound_mask(const uint32_t& K_idx_lane_base,
                                                        float RS[][num_tiles_k][8],
...
template <uint32_t num_tiles_q, uint32_t num_tiles_k, uint32_t num_tiles_v, bool exp_offset,
          bool fuse_scale = false>
__device__ __forceinline__ void update_mdo(float RS[][num_tiles_k][8], float RO[][num_tiles_v][8],
                                           float m[][2], float d[][2], float pv_scale[][2],
                                           const float& sm_scale, const float exp_offset_value)
{
#pragma unroll
        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
        {
...
#pragma unroll
                        ...
                        {
                                const float m_local =
                                    max(max(RS[fq][fk][k * 2 + 0], RS[fq][fk][k * 2 + 1]),
                        ...
                        const float smaller_scale = math::ptx_exp2(-fabsf(m_prev - tile_m));
...
#pragma unroll
                        for (uint32_t fk = 0; fk < num_tiles_k; fk++)
                        {
                                ...
                                {
                                        RS[fq][fk][k * 2 + 0] = math::ptx_exp2(
                                            fmaf(RS[fq][fk][k * 2 + 0], sm_scale, negative_m));
                                        RS[fq][fk][k * 2 + 1] = math::ptx_exp2(
                                            fmaf(RS[fq][fk][k * 2 + 1], sm_scale, negative_m));
                                        RS[fq][fk][k * 2 + 4] = math::ptx_exp2(
                                            fmaf(RS[fq][fk][k * 2 + 4], sm_scale, negative_m));
                                        RS[fq][fk][k * 2 + 5] = math::ptx_exp2(
                                            fmaf(RS[fq][fk][k * 2 + 5], sm_scale, negative_m));
                                }
                        }
                }
        }
...
template <uint32_t num_tiles_q, uint32_t num_tiles_k>
__device__ __forceinline__ void RS_32_to_8(float RS[][num_tiles_k][8],
                                           uint32_t RS_8[][num_tiles_k / 2][4])
{
#pragma unroll
        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
        {
...
__device__ __forceinline__ PackedU8RowSum pack_scaled_exp2_u8x4(const int32_t a, const int32_t b,
                                                                const int32_t c, const int32_t d,
                                                                const float scale,
                                                                const float negative_m)
{
        const float probability_a = math::ptx_exp2(fmaf(__int2float_rz(a), scale, negative_m));
        const float probability_b = math::ptx_exp2(fmaf(__int2float_rz(b), scale, negative_m));
        const float probability_c = math::ptx_exp2(fmaf(__int2float_rz(c), scale, negative_m));
...
__device__ __forceinline__ void update_mdo_i32_u8(int32_t RS[][num_tiles_k][8],
                                                  float RO[][num_tiles_v][8], float m[][2],
                                                  float d[][2], const float sm_scale,
...
#pragma unroll
                        for (uint32_t fk = 0; fk < num_tiles_k / 2; fk++)
                        {
                                const PackedU8RowSum probabilities_0 = pack_scaled_exp2_u8x4(
                                    ...
                                    RS[fq][fk * 2][k * 2 + 4], RS[fq][fk * 2][k * 2 + 5], sm_scale,
                                    negative_m);
                                const PackedU8RowSum probabilities_1 = pack_scaled_exp2_u8x4(
                                    RS[fq][fk * 2 + 1][k * 2], RS[fq][fk * 2 + 1][k * 2 + 1],
                                    RS[fq][fk * 2 + 1][k * 2 + 4], RS[fq][fk * 2 + 1][k * 2 + 5],
                                    sm_scale, negative_m);
...
template <uint32_t num_tiles_q, uint32_t num_tiles_k, typename T>
__device__ __forceinline__ void RS_to_u8(T RS[][num_tiles_k][8],
                                         uint32_t RS_u8[][num_tiles_k / 2][4])
{
#pragma unroll
        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
...
#pragma unroll
                for (uint32_t fk = 0; fk < num_tiles_k / 2; fk++)
                {
                        RS_u8[fq][fk][0] = pack_u8x4(RS[fq][fk * 2][0], RS[fq][fk * 2][1],
                                                     RS[fq][fk * 2][4], RS[fq][fk * 2][5]);
                        RS_u8[fq][fk][1] = pack_u8x4(RS[fq][fk * 2][2], RS[fq][fk * 2][3],
                                                     RS[fq][fk * 2][6], RS[fq][fk * 2][7]);
                        RS_u8[fq][fk][2] = pack_u8x4(RS[fq][fk * 2 + 1][0], RS[fq][fk * 2 + 1][1],
                                                     RS[fq][fk * 2 + 1][4], RS[fq][fk * 2 + 1][5]);
                        RS_u8[fq][fk][3] = pack_u8x4(RS[fq][fk * 2 + 1][2], RS[fq][fk * 2 + 1][3],
...
template <uint32_t num_tiles_q, uint32_t num_tiles_k>
__device__ __forceinline__ void RS_16_to_8(uint32_t RS[][num_tiles_k][4],
                                           uint32_t RS_8[][num_tiles_k / 2][4])
{
#pragma unroll
        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
...
template <uint32_t num_tiles_q, uint32_t num_tiles_k>
__device__ __forceinline__ void accumulate_d(float RS[][num_tiles_k][8], float d[][2],
                                             float pv_scale[][2])
{
#pragma unroll
        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
...
#pragma unroll
                for (uint32_t fk = 0; fk < num_tiles_k; fk++)
                {
                        const float tile_sum_0 =
                            (RS[fq][fk][0] + RS[fq][fk][1]) + (RS[fq][fk][4] + RS[fq][fk][5]);
                        const float tile_sum_1 =
                            (RS[fq][fk][2] + RS[fq][fk][3]) + (RS[fq][fk][6] + RS[fq][fk][7]);
...
template <uint32_t num_warps_q, uint32_t num_warps_k, uint32_t num_tiles_q, uint32_t num_tiles_k,
          uint32_t num_tiles_v, SwizzleMode swizzle_mode, uint32_t stride, typename DTypeSVAccum>
                                                ...
                                                DTypeSVAccum RO[][num_tiles_v][8], float d[][2])
{
        uint32_t smem_V_row_base =
            get_warp_idx_k<num_warps_q, num_warps_k>() * (num_tiles_k * 16) + get_lane_id() % 16;
...
#pragma unroll
                for (uint32_t fv = 0; fv < num_tiles_v; fv++)
                {
                        // load RV
                        uint32_t RV[4];
                        uint32_t offset_V = (smem_V).get_permuted_offset(smem_V_row_base + fk * 16,
                                                                         smem_V_col_base + fv * 2);
...
#pragma nv_diag_suppress 174
template <uint32_t num_warps_q, uint32_t num_warps_k, uint32_t num_tiles_q, uint32_t num_tiles_k,
          uint32_t num_tiles_v, SwizzleMode swizzle_mode, uint32_t stride, uint32_t RS_width = 4,
          typename T, typename DTypeSVAccum>
...
template <uint32_t num_warps_q, uint32_t num_warps_k, uint32_t num_tiles_q, uint32_t num_tiles_k,
          uint32_t num_tiles_v, SwizzleMode swizzle_mode, uint32_t stride, uint32_t RS_width = 4,
          typename T, typename DTypeSVAccum>
__device__ __forceinline__ void compute_fp16_sv_permuted_inst_buf(
    const smem_t<swizzle_mode, stride>& smem_V, T RS_f16[][num_tiles_k][RS_width],
    DTypeSVAccum RO[][num_tiles_v][8], float d[][2], uint32_t& offset_V)
{
        static_assert(sizeof(T) == 4);
...
#pragma unroll
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                {
                                        mma::mma_sync_m16n16k16_row_col_f16f16f16<
                                            mma::MMAMode::kInit>((uint32_t*)RO_inst_buf[fq][fv],
...
__device__ __forceinline__ void normalize_d(float RO[][num_tiles_v][8], float m[][2], float d[][2])
{
...
template <uint32_t num_warps_q, uint32_t num_warps_k, uint32_t num_tiles_q, uint32_t num_tiles_k,
          uint32_t num_tiles_v, SwizzleMode swizzle_mode, uint32_t stride, typename DTypeSVAccum>
__device__ __forceinline__ void compute_int8_sv(const smem_t<swizzle_mode, stride>& smem_V,
...
#pragma unroll
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                if constexpr (std::is_same<DTypeSVAccum, float>::value)
                                {
                                        mma::mma_sync_m16n16k32_row_col_f8f8f32(RO[fq][fv],
                                                                                RS_f8[fq][fk], RV);
                                }
                                else if constexpr (std::is_same<DTypeSVAccum, half>::value)
                                {
                        ...
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                if constexpr (std::is_same<DTypeSVAccum, float>::value)
                                {
                                        mma::mma_sync_m16n16k32_row_col_f8f8f32<
                                            mma::MMAMode::kInit>(RO_inst_buf[fq][fv], RS_f8[fq][fk],
                                                                 RV);
                                }
                                else if constexpr (std::is_same<DTypeSVAccum, half>::value)
                                {
...
#pragma unroll
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                ...
                                {
                                        mma::mma_sync_m16n16k32_row_col_f8f8f32<
                                            mma::MMAMode::kInplaceUpdate>(RO_inst_buf[fq][fv],
                                                                          RS_f8[fq][fk], RV);
                                }
                                else if constexpr (std::is_same<DTypeSVAccum, half>::value)
                                {
...
template <uint32_t num_warps_q, uint32_t num_warps_k, uint32_t num_tiles_q, uint32_t num_tiles_k,
          uint32_t num_tiles_v, SwizzleMode swizzle_mode, uint32_t stride, typename DTypeSVAccum>
__device__ __forceinline__ void compute_fp8_sv_inst_buf_fp16_accu(
    const smem_t<swizzle_mode, stride>& smem_V, uint32_t RS_f8[][num_tiles_k / 2][4],
    DTypeSVAccum RO[][num_tiles_v][8], float d[][2])
{
        uint32_t smem_V_row_base = get_lane_id() % 8 + (get_lane_id() / 16) * 8;
        // uint32_t smem_V_col_base = get_warp_idx_k<num_warps_q, num_warps_k>() *
                        ...
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                if constexpr (std::is_same<DTypeSVAccum, float>::value)
                                {
                                        ...
                                        mma::mma_sync_m16n16k32_row_col_f8f8f16<
                                            mma::MMAMode::kInit>(RO_int32[fq][fv], RS_f8[fq][fk],
                                                                 RV);
                                }
                                else if constexpr (std::is_same<DTypeSVAccum, half>::value)
                                {
...
#pragma unroll
                        for (uint32_t fq = 0; fq < num_tiles_q; fq++)
                        {
                                if constexpr (std::is_same<DTypeSVAccum, float>::value)
                                {
                                        ...
                                        // RS_f8[fq][fk], RV);
                                        mma::mma_sync_m16n16k32_row_col_f8f8f16<
                                            mma::MMAMode::kInplaceUpdate>(RO_int32[fq][fv],
                                                                          RS_f8[fq][fk], RV);
                                }
                                else if constexpr (std::is_same<DTypeSVAccum, half>::value)
                                {
