...
__global__ void quant_v_int8_kernel(const T* __restrict__ v, int8_t* __restrict__ out,
...
extern "C" void launch_quant_v_int8_kernel(const void* v, void* out, void* scale, int B, int H,
                                           int N, int D, int padded_N, int64_t sb, int64_t sh,
                                           int64_t sn, int input_dtype_code, hipStream_t stream)
{
        ...
        if (reinterpret_cast<uintptr_t>(v) % 16 != 0 || !stride_ok(sb, B) || !stride_ok(sh, H) ||
            !stride_ok(sn, N))
        {
