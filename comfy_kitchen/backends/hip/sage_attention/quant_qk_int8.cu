...
__forceinline__ __device__ void apply_convrot_sign128(float* values, const int lane)
{
...
#pragma unroll
                for (int c = 0; c < 4; ++c)
                {
                        ...
                        values[c] = (half_lane & bit) ? other - values[c] : values[c] + other;
                }
        }
...
template <typename T, int NR, int BLKQ, int WARPQ, int CHANNEL_TILES, int ROTATION, bool ALIGNED4>
__forceinline__ __device__ void process_q(const T* __restrict__ in, int8_t* __restrict__ out,
...
#pragma unroll
                                for (int j = 0; j < NR; ++j)
                                {
                                        ...
                                        if (n < L)
                                        {
                                                VectorLoader4<T>::load(
                                                    &in[(int64_t)n * stride_n + ch], &v[vi]);
                                                mx = fmaxf(
                                                    mx, fmaxf(fmaxf(fabsf(v[vi]), fabsf(v[vi + 1])),
                                                              fmaxf(fabsf(v[vi + 2]),
                                                                    fabsf(v[vi + 3]))));
                                        }
                                }
...
#pragma unroll
                ...
                if (lane == 0) sc_buf[oblk * 8 + otld] = sc;
...
// ---------------------------------------------------------------------------
template <typename T, int NL, int WARPK, int CHANNEL_TILES, int ROTATION, bool ALIGNED4>
__forceinline__ __device__ void process_k(const T* __restrict__ in, int8_t* __restrict__ out,
                                          float* __restrict__ sc_buf, const int oblk, const int L,
                                          const int C, const int anchor_index,
...
#pragma unroll
                ...
                {
                        const int ch = tile * 128 + (lane << 2);
                        const int64_t anchor_offset = (int64_t)anchor_index * stride_n + ch;
                        if (ALIGNED4 || ch + 3 < C)
                        {
...
#pragma unroll
        ...
        if (lane == 0) sc_buf[oblk * 4 + otld] = sc;
...
__global__ __launch_bounds__(CENTER_DETECT_THREADS) void detect_k_anchor(
    const T* __restrict__ k_in, int* __restrict__ anchor_indices, const int Lk, const int C,
    const int H_kv, const int64_t stride_b, const int64_t stride_h, const int64_t stride_n)
{
        ...
        for (int index = tid; index < CENTER_SAMPLES * C; index += CENTER_DETECT_THREADS)
        {
...
#pragma unroll
                ...
                anchor_indices[b * H_kv + h] =
                    improves_range ? selected_candidate * (Lk - 1) / (CENTER_SAMPLES - 1) : -1;
        }
}
...
__global__ __launch_bounds__(128, 4) void quant_q_kernel(
    const T* __restrict__ q_in, int8_t* __restrict__ q_out, float* __restrict__ q_sb, const int Lq,
    const int C, const int H_q, const int q_sc_per_h, const int64_t stride_b,
...
__global__ __launch_bounds__(128, 4) void quant_k_kernel(
    const T* __restrict__ k_in, int8_t* __restrict__ k_out, float* __restrict__ k_sb,
    const int* __restrict__ anchor_indices, const int Lk, const int C, const int H_kv,
    const int k_sc_per_h, const int64_t stride_b, const int64_t stride_h, const int64_t stride_n)
{
        const int oblk = blockIdx.x;
        const int h = blockIdx.y, b = blockIdx.z;
        const int64_t in_bh = (int64_t)b * stride_b + (int64_t)h * stride_h;
...
// ---------------------------------------------------------------------------
template <typename T, int NR, int NL, int BLKQ, int WARPQ, int BLKK, int WARPK, int CHANNEL_TILES,
          int ROTATION, bool ALIGNED4>
__global__ __launch_bounds__(128, 3) void quant_qk_fused(
    const T* __restrict__ q_in, int8_t* __restrict__ q_out, float* __restrict__ q_sb,
    const T* __restrict__ k_in, int8_t* __restrict__ k_out, float* __restrict__ k_sb,
    const int* __restrict__ anchor_indices, const int Lq, const int Lk, const int C,
    ...
    const int64_t q_stride_n, const int64_t k_stride_b, const int64_t k_stride_h,
    const int64_t k_stride_n)
{
        ...
        if (blockIdx.x < (unsigned)q_oblk_count)
        {
                ...
                const int64_t sbh = ((int64_t)b * H_q + h) * q_sc_per_h;
                process_q<T, NR, BLKQ, WARPQ, CHANNEL_TILES, ROTATION, ALIGNED4>(
                    q_in + in_bh, q_out + out_bh, q_sb + sbh, blockIdx.x, Lq, C, q_stride_n);
        }
        else
        {
                if (h >= H_kv) return;
...
extern "C" void launch_quant_qk_per_thread_int8(const void* q, void* q_int8, void* q_scale,
                                                const void* k, void* k_int8, void* k_scale, int B,
                                                int H_q, int Lq, int H_kv, int Lk, int C, int BLKQ,
                                                int WARPQ, int BLKK, int WARPK, int64_t q_stride_b,
                                                int64_t q_stride_h, int64_t q_stride_n,
                                                int64_t k_stride_b, int64_t k_stride_h,
                                                int64_t k_stride_n, int input_dtype_code,
                                                void* anchor_indices, hipStream_t stream)
{
        if (C != 64 && C != 128 && C != 256)
        {
        ...
        if (BLKQ != 128 || WARPQ != expected_warpq || (BLKK != 64 && BLKK != 128) ||
            WARPK != BLKK || (C == 64 && BLKK != 64))
        {
                throw std::runtime_error(
        ...
        const auto is_vector_aligned =
            [vector_size, &stride_ok](const void* ptr, int64_t stride_b, int64_t stride_h,
                                      int64_t stride_n, int extent_b, int extent_h, int extent_n)
        {
        ...
        {
                throw std::runtime_error(
                    "quant_qk_per_thread_int8: Q/K base pointers and B/H/N strides must preserve "
                    "4-element alignment");
        }
        ...
        hipError_t anchor_error = hipGetLastError();
        if (anchor_error != hipSuccess)
        {
                throw std::runtime_error(std::string("detect_k_anchor kernel launch failed: ") +
                                         hipGetErrorString(anchor_error));
        }
...
#define LAUNCH_SPLIT(T, NR, NL, BQ, WQ, BK, WK, CT, ROT, A4)                                       \
        ...
        {                                                                                          \
                ...
                quant_q_kernel<T, NR, BQ, WQ, CT, ROT, A4>                                         \
                    <<<gq, 128, 0, stream>>>((const T*)q, (int8_t*)q_int8, (float*)q_scale, Lq, C, \
                                             H_q, q_sc_per_h, q_stride_b, q_stride_h, q_stride_n); \
                hipError_t q_error = hipGetLastError();                                            \
                if (q_error != hipSuccess)                                                         \
                        throw std::runtime_error(std::string("quant_q kernel launch failed: ") +   \
                                                 hipGetErrorString(q_error));                      \
                dim3 gk(k_oblk, H_kv, B);                                                          \
                quant_k_kernel<T, NL, WK, CT, ROT, A4><<<gk, 128, 0, stream>>>(                    \
                    (const T*)k, (int8_t*)k_int8, (float*)k_scale, anchor_ptr, Lk, C, H_kv,        \
                    k_sc_per_h, k_stride_b, k_stride_h, k_stride_n);                               \
                hipError_t k_error = hipGetLastError();                                            \
                if (k_error != hipSuccess)                                                         \
                        throw std::runtime_error(std::string("quant_k kernel launch failed: ") +   \
                                                 hipGetErrorString(k_error));                      \
        } while (0)
...
#define LAUNCH_FUSED(T, NR, NL, BQ, WQ, BK, WK, CT, ROT, A4)                                     \
        ...
        {                                                                                        \
                ...
                quant_qk_fused<T, NR, NL, BQ, WQ, BK, WK, CT, ROT, A4><<<g, 128, 0, stream>>>(   \
                    (const T*)q, (int8_t*)q_int8, (float*)q_scale, (const T*)k, (int8_t*)k_int8, \
                    (float*)k_scale, anchor_ptr, Lq, Lk, C, q_oblk, H_q, H_kv, q_sc_per_h,       \
                    k_sc_per_h, q_stride_b, q_stride_h, q_stride_n, k_stride_b, k_stride_h,      \
                    k_stride_n);                                                                 \
                hipError_t qk_error = hipGetLastError();                                         \
                if (qk_error != hipSuccess)                                                      \
                        throw std::runtime_error(                                                \
                            std::string("quant_qk fused kernel launch failed: ") +               \
                            hipGetErrorString(qk_error));                                        \
        } while (0)

#define LAUNCH_C64(T, ROT) LAUNCH_FUSED(T, 4, 8, 128, 32, 64, 64, 1, ROT, false)

#define LAUNCH_C128_CTA64(T, ROT) LAUNCH_FUSED(T, 4, 8, 128, 32, 64, 64, 1, ROT, true)

#define LAUNCH_C128_CTA128(T, ROT) LAUNCH_FUSED(T, 4, 16, 128, 32, 128, 128, 1, ROT, true)

#define LAUNCH_C256_CTA64(T, ROT) LAUNCH_SPLIT(T, 2, 8, 128, 16, 64, 64, 2, ROT, true)

#define LAUNCH_C256_CTA128(T, ROT) LAUNCH_SPLIT(T, 2, 16, 128, 16, 128, 128, 2, ROT, true)
